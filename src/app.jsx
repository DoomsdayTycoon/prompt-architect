...
- Every cell must be complete -- no truncation, no word-breaking, max 2 lines per cell.`;
  ...
  const FORMULA_RULES=`FORMULA & EQUATION RULES (apply to every mathematical expression):
- ALWAYS use LaTeX syntax for math. NEVER use ASCII fallbacks like x^alpha, >=, <=, !=, approx, sum, integral, sqrt, lambda, beta, <=, or unicode soup.
- Inline math: wrap in single dollars: $v(x) = x^{\\alpha}$. Display math: wrap in double dollars on its own line: $$v(x) = \\begin{cases} x^{\\alpha} & x \\geq 0 \\\\ -\\lambda(-x)^{\\beta} & x < 0 \\end{cases}$$
- Use proper LaTeX primitives: \\alpha \\beta \\gamma \\lambda \\mu \\sigma \\Sigma \\Delta \\pi \\theta \\rho \\epsilon \\phi \\Phi \\psi \\Psi \\omega \\Omega; \\leq \\geq \\neq \\approx \\equiv \\pm \\times \\cdot \\div \\infty \\partial \\nabla; \\sum \\int \\prod \\lim \\frac{a}{b} \\sqrt{x} \\sqrt[n]{x}; subscripts x_{i}, superscripts x^{n}, multi-char indices with braces: x_{i,j}^{2}.
- Align multi-step derivations with \\begin{aligned}...\\end{aligned} and & anchors. Number key equations with \\tag{1}, \\tag{2} and reference them as "see eq. (1)".
- Variable definitions: immediately after first use, define every symbol in a "where:" block with one bullet per symbol.
- Units OUTSIDE the math: $E = 42$ kWh, NOT $E = 42\\,\\text{kWh}$ mixed inline. For unit-inside-math use \\text{} or \\mathrm{}.
- Percentages in math: use \\% (0.88 \\approx 88\\%), never a bare % inside $$.
- Matrices: \\begin{bmatrix}...\\end{bmatrix}. Fractions: \\frac{num}{den}, NEVER inline slashes for complex fractions. Nested fractions: \\dfrac for display size.
- Citations next to equations: put author-year AFTER the equation block, not inside the math, e.g. "...(Tversky & Kahneman, 1992)."
- RENDERING TARGET: the document will be rendered by a LaTeX-aware pipeline (MathJax / KaTeX / pdflatex / Word Equation). Produce LaTeX that parses cleanly in all three. No custom macros, no \\newcommand, no \\usepackage. Stick to amsmath primitives.
- Formulas must look like they came from a published textbook, not a chat transcript. If you find yourself typing "^" or "_" outside of $...$, STOP and wrap it in math mode.`;
...
STANDARDS: Zero inline styles. 2-space indent. No deprecated elements. Print stylesheet (@media print). Lazy-load images. Load MathJax v3 for LaTeX rendering: <script src="https://cdn.jsdelivr.net/npm/mathjax@3/es5/tex-mml-chtml.js"></script>.
`+FORMULA_RULES,
            ...
            return (
              <div key={h.id||i} onClick={()=>loadFromHistory(h)} style={{position:"relative",padding:"12px 44px 12px 14px",borderRadius:10,border:"1px solid var(--bd)",background:"var(--s2)",cursor:"pointer",fontFamily:"var(--f)",transition:"all .15s",borderLeft:"3px solid "+mc}} onMouseEnter={e=>{e.currentTarget.style.background="var(--bg)";e.currentTarget.style.borderColor=mc+"55";}} onMouseLeave={e=>{e.currentTarget.style.background="var(--s2)";e.currentTarget.style.borderColor="var(--bd)";e.currentTarget.style.borderLeftColor=mc;}}>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:5,alignItems:"center"}}>
                  ...
                  {h.fileOutput&&FILE_OUTPUTS[h.fileOutput]&&chip(FILE_OUTPUTS[h.fileOutput].l,"var(--t3)","var(--bg)")}
                </div>
