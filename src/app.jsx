...
function buildPrompt(p){
  ...
  // ═══ BLOCK 1: SYSTEM — Role + expertise + behavioral constraints ═══
  const roleParts=[];
  roleParts.push("You are "+(ROLE_DEEP[taskType]||"a helpful expert assistant")+".");
  if(industry!=="general"&&IND_CONTEXT[industry])roleParts.push(IND_CONTEXT[industry]);
  if(selectedFirm&&SECTOR_FIRMS[industry]?.firms[selectedFirm]){
    const firmData=SECTOR_FIRMS[industry].firms[selectedFirm];
    roleParts.push("Adopt the methodology, rigor, and deliverable quality of "+firmData.name+".");
    if(selectedRole)roleParts.push("You are operating as a "+selectedRole+" at "+firmData.name+". Apply this role's expertise and perspective.");
  }
  const audienceEnhancement=(getSmartEnhancements(topic,taskType,industry,output)).find(e=>e.type==="audience");
  if(audience)roleParts.push("Target audience: "+audience+".");
  else if(audienceEnhancement)roleParts.push(audienceEnhancement.text);
  else roleParts.push("Target audience: knowledgeable professionals who expect depth and precision.");
  roleParts.push("Tone: "+tone+(TONE_INST[tone]?" -- "+TONE_INST[tone]:"")+".");
  if(language&&language!=="English")roleParts.push("Respond entirely in "+language+".");
  sec("system",roleParts.join("\n"));

  // ═══ BLOCK 2: TASK — What to produce, with numbered deliverables ═══
  ...
  const tierSuffix=tier==="flagship"
    ?"\n\nDeliver expert-level depth and genuine professional value -- not a surface-level overview."
    :tier==="balanced"
    ?"\n\nBe thorough but efficient. Prioritize actionable insights over exhaustive detail."
    :"\n\nBe concise and direct. Focus on the most important points. Skip preamble.";
  const taskBlock=sharpTopic+(smartContext?"\n\n"+smartContext:"")+tierSuffix;
  sec("task",taskBlock);

  // Source guidance (compact)
  if(hasAttachment){
    sec("sources","Files are attached. Use them as PRIMARY data source. Extract data directly, reference specific figures. State gaps explicitly.");
  }else{
    sec("sources","No files attached. Use realistic, well-researched figures. Label estimates clearly. Note what source data would strengthen the analysis.");
  }

  // Methodology (compact, adapted to tier)
  const methods=METHODOLOGY[taskType]||METHODOLOGY.writing;
  if(tier==="fast"){
    sec("approach","Steps: "+methods.slice(0,3).map((m,i)=>(i+1)+". "+m).join(" "));
  }else{
    sec("approach",methods.map((m,i)=>(i+1)+". "+m).join("\n"));
  }

  // Risk level (only for non-medium)
  const risk=riskLevel||"medium";
  if(risk==="high")sec("risk_level","HIGH RISK: Validate every assumption. Flag irreversible actions. Consider edge cases and failure modes. Include rollback procedures.");
  else if(risk==="low")sec("risk_level","LOW RISK: Optimize for speed. Skip extensive caveats. Be concise and action-oriented.");

  // Includes (required sections)
  if(includes.length>0){
    const incInst=[];
    if(includes.includes("exec_summary"))incInst.push("Executive Summary (3-5 sentences) upfront");
    if(includes.includes("examples"))incInst.push("2-3 real-world examples with named companies or plausible case studies");
    if(includes.includes("sources"))incInst.push("Source citations for factual claims -- named reports, studies, publications");
    if(includes.includes("action_items"))incInst.push("Action Items / Next Steps: numbered, specific, with owners and timelines");
    if(includes.includes("risks"))incInst.push("Risk Assessment: top 3-5 risks with likelihood, impact, mitigation");
    if(includes.includes("comparison"))incInst.push("Comparison Table evaluating options on same criteria");
    if(includes.includes("timeline"))incInst.push("Timeline / Milestones with phases, deliverables, dates");
    if(includes.includes("metrics"))incInst.push("KPIs / Metrics: baseline, target, measurement method");
    if(includes.includes("glossary"))incInst.push("Glossary of technical terms and acronyms");
    if(includes.includes("visuals"))incInst.push("Recommended visuals: chart type, data shown, purpose");
    sec("required_sections","Must include:\n"+incInst.map((x,i)=>"- "+x).join("\n"));
  }

  // Smart enhancements (technique/comparison/precision)
  ...
  if(techniques.includes("constraints"))sec("constraints","1. Never fabricate facts or sources\n2. State uncertainty explicitly with confidence level\n3. Stay focused -- no tangential drift\n4. Distinguish facts from analysis from speculation\n5. Flag incorrect assumptions before proceeding");
  if(techniques.includes("selfcheck"))sec("self_check","Before submitting, verify: fully addresses task, clear structure, claims supported, tone consistent, no repetition. Revise any section that fails.");
  if(techniques.includes("compare"))sec("comparative","Present each option with clear label. Evaluate on same criteria. Use structured comparison for 3+ options. State recommendation with reasoning.");
  if(techniques.includes("iterative"))sec("refinement","After initial draft: re-read as target audience, identify gaps, provide improved version, note what changed.");
  if(techniques.includes("roleplay"))sec("persona","Embody this role fully. Use vocabulary and reasoning natural to it. Maintain consistent perspective throughout.");
  if(techniques.includes("redteam"))sec("red_team","After your main response, switch to adversarial mode. Attack your own work: weakest arguments, wrong assumptions, what a smart opponent would say. Then strengthen your response.");
  if(techniques.includes("firstprinciples"))sec("first_principles","Strip to fundamental truths. Question every assumption. If solving from scratch with no precedent, what would you do? Rebuild from foundations.");
  if(techniques.includes("inversion"))sec("inversion","Invert the problem: imagine catastrophic failure 12 months out. Top 5 failure modes, early warning signs, prevention actions. Use pre-mortem to strengthen the recommendation.");
  if(techniques.includes("multiagent"))sec("multi_perspective","Analyze from multiple expert perspectives (e.g., CFO, customer, regulator, engineer). State each role's priorities and risks. Note conflicts. Synthesize into a recommendation.");
  if(techniques.includes("structured_debate"))sec("debate","Present as structured debate:\n**Position A (FOR):** Strongest case, 3-5 arguments with evidence\n**Position B (AGAINST):** Strongest countercase\n**Verdict:** Which is stronger and why, with nuance.");
  if(techniques.includes("systems"))sec("systems","Map as a system: key components, feedback loops, leverage points, time delays, unintended side effects. Draw connections narrow analysis would miss.");
  if(techniques.includes("decompose"))sec("decompose","Break into sub-tasks. Identify dependencies. For each: approach, expected output, success criteria. Solve systematically, integrate, verify against requirements.");
  if(techniques.includes("meta"))sec("meta","Before diving in: What type of problem is this? Best frameworks? Biggest risk of getting wrong? Likely blind spots? Let this guide your approach.");

  // Extra & special
  if(extra)sec("additional_instructions",extra);
  if(special)sec("special_requests",special);

  // ═══ BLOCK 3: CONTRACT — Output format, structure, acceptance criteria ═══
  const fmtArr=Array.isArray(format)?format:[format];
  const FMT_INST={
    prose:"Flowing paragraphs with topic sentences and logical transitions. Subheadings for major sections.",
    bullets:"Concise bullet points under clear headers. Each bullet: 1-2 sentences, lead with most important info.",
    numbered:"Numbered steps with bold action verbs. Brief explanations under each step.",
    table:`Well-labeled tables with clear headers. Rules:
  - Max 6 columns portrait, abbreviate headers (Rev., Gr., Mgn., FY25E)
  - Numbers: right-aligned, commas for thousands, 1 decimal %, negatives in ()
  - Empty cells: em dash (--), never N/A or 0
  - Units as subtitle below title, never inline
  - Source line below every table`,
    qa:"Questions followed by thorough answers. Foundational to advanced order. Self-contained pairs.",
    headers:"Clear header hierarchy (H2 major, H3 sub). Scannable sections. TOC for longer responses.",
    tldr:"TL;DR (3-5 sentences) first, then full detail. TL;DR must stand alone.",
    framework:"Structure around a recognized framework. Name it, explain why it applies, map analysis to its components.",
    flowchart:"Text-based decision trees with arrows and branching. Label each decision point and outcome.",
    before_after:"Before / After comparisons showing current state vs. recommended state with concrete examples.",
    pros_cons:"Structured Pros & Cons for each recommendation. Specific and quantified where possible.",
    scorecard:"Scorecard/rating matrix. Consistent scale (1-5 or H/M/L). Use abbreviations to fit columns. Equal-width rating columns.",
    annotated:"Annotated examples walking through scenarios step by step with inline commentary."
  };
  const contractParts=[];

  // Output type
  if(output&&OUTPUT_INST[output])contractParts.push(OUTPUT_INST[output]);
  ...
  if(fmtArr.length>0){
    contractParts.push("Format:\n"+fmtArr.map((f,i)=>"- **"+(FMTS[f]?.l||f)+"**: "+(FMT_INST[f]||"")).join("\n"));
  }

  // Length
  const fileLenSub=getLenSub(length,fileOutput);
  const lenG={Brief:"200-400 words. Highest-impact only.",Medium:"500-900 words. All major aspects with supporting detail.",Detailed:"1,000-1,800 words. Thorough with examples and nuance.",Comprehensive:"2,000+ words. Exhaustive reference-grade depth."};
  contractParts.push("Length: "+fileLenSub+". "+(lenG[length]||lenG.Medium));

  // FILE OUTPUT FORMAT — Compressed professional-grade specs
  const FILE_INST={
    pdf:`FORMAT: PDF, institutional quality (Goldman Sachs / J.P. Morgan standard)
STRUCTURE: Title page (title 28pt, subtitle 16pt, date Month DD YYYY, classification) -- TOC -- Numbered sections (1.0, 1.1) -- Glossary
TYPOGRAPHY: Arial throughout. Body 10.5pt #333. H1 20pt bold #1B2A4A with rule below. H2 14pt bold #1B2A4A. H3 11pt bold #333. Margins 1in L/R, 0.75in T/B. 6pt paragraph spacing. Left-aligned (not justified). Callouts: #EBF5FB bg, 1pt navy left border.
TABLES:
  - Header: 10pt bold, white on #1B2A4A, 10px padding
  - Data: 9.5pt, 8px padding, alternating white/#F8F9FA rows
  - Borders: 0.5pt #D0D5DD, 1pt navy under header
  - Max 6 columns portrait, 10 landscape. Split if wider.
  - Numbers: right-aligned, commas for thousands, 1 decimal %, 2 decimal currency, negatives in ()
  - Empty = em dash (--). Never N/A, None, or 0. Remove all-empty columns.
  - Units (NOK '000, USD millions): 8pt italic subtitle below title, never inline
  - Currency/$: first and total rows only
  - Title above: "Table N: [Title]". Source below: 8pt italic.
  - Every cell complete -- no truncation, no word-breaking, max 2 lines
  - Abbreviate headers (Rev., Gr., Mgn., FY25E). Glossary at end.
CHARTS: Title as insight statement. Navy/steel blue/teal palette, max 5 colors. 8pt axis labels. Data labels on bars. Source below.
PAGE FLOW: Hard break before H1. Never split table from title. Min 2 rows per page side. If <30% page left, start table on next page. No page <40% filled.`,

    word:`FORMAT: DOCX, Big 4 consulting / top-tier law firm standard
STRUCTURE: Title page (Calibri 26pt, date, version, confidentiality) -- TOC via Heading styles -- Section numbering (1.0, 1.1) -- Hard break before H1
TYPOGRAPHY: Calibri throughout. Body 11pt #333, 1.25 spacing, 6pt after. H1 18pt bold #1B2A4A with bottom border. H2 14pt bold. H3 11.5pt bold. Callouts: #EBF5FB bg, navy left border.
TABLES:
  - Header: 10pt bold, white on #1B2A4A, repeat on multi-page tables
  - Data: 10pt, 6px padding, alternating white/#F8F9FA
  - Numbers: right-aligned, commas, 1 decimal %, negatives in ()
  - Empty = em dash (--). Never N/A or 0. Remove all-empty columns.
  - Title above, source below. Auto-fit columns, min 60px.
  - Every cell complete -- no truncation, max 2 lines
  - Abbreviate headers. Glossary at end.
  - Landscape section break if wider than margins
PAGE FLOW: Table title + body on same page. Never split with <2 rows per side. Source on same page as table.`,

    excel:`FORMAT: XLSX, investment-bank-grade (Goldman Sachs / J.P. Morgan standard)
STRUCTURE: Numbered tabs ("1. Summary", "2. Revenue Build", etc.). One purpose per tab. Summary tab first with navigation.
LAYOUT: Row 1-3 title block (company, title, date). Row 5+ data. Freeze Row 5 + Col A. Hide gridlines.
  - Headers: 10pt bold, white on #1B2A4A, 22px height
  - Data: 9.5pt, 18px height, alternating white/#F8F9FA
  - Labels LEFT, numbers RIGHT. Auto-fit + 20px padding. Never allow ###.
NUMBERS (IB standard):
  - Millions: 1 decimal, $ on first/total rows only
  - Percentages: 1 decimal + %. Multiples: 1 decimal + x. Per-share: 2 decimal.
  - Commas for thousands. Negatives: red () no minus. Growth: +prefix.
  - Empty = em dash (--). Never N/A, None, or 0.
  - Units in subtitle cell (8pt italic gray), never inline with title.
CELL TYPES: Input = blue bg (#DCE6F1). Calculated = formula only, no bg. Linked = green font (#006100). Override = orange (#E26B0A) + comment.
FORMULAS: Zero hardcoded values. Named ranges for assumptions. IFERROR on divisions. Scenario toggles via dropdown.
COLORING: Section totals: bold, 1pt navy top, #E8EEF4 bg. Grand totals: double border. Negatives: red #CC0000. Positives: green #006100. KPI status: green/amber/red conditional formatting.
COMPONENTS: Revenue bridge, sensitivity tables (2-var with base case bold), comps table, DCF (WACC build, FCF, terminal value, EV bridge), IRR/MOIC.
CHARTS: Insight titles. Navy/steel blue/teal, max 5 colors. 8pt labels. Data labels on bars.`,

    ppt:`FORMAT: PPTX, pitchbook standard (Goldman Sachs / McKinsey quality), 16:9
STRUCTURE: Title slide (28pt white on navy, subtitle, date, confidentiality) -- Agenda -- Section dividers (navy bg) -- Content slides -- Summary + Next Steps -- Appendix
SLIDE RULES:
  - Assertion-Evidence format: title IS the conclusion, body proves it with data
...
TYPOGRAPHY: Title 20pt bold #1B2A4A, max 2 lines. Body 14pt #333. Bullets 13pt em dash. Sub-bullets 11pt. Min 8pt for sources.
TABLES ON SLIDES:
  - Max 7 rows x 6 columns. Split or appendix if more.
  - Header: 10pt bold, white on navy. Data: 9.5pt, alternating rows.
  - Horizontal borders only. Numbers right-aligned, IB formatting.
  - Every cell complete. Abbreviate aggressively. Centered on slide.
...
SPEAKER NOTES: Key message, 3-4 data points, anticipated pushback, transition to next slide.
NEVER: animations, transitions, clip art, font below 8pt, misaligned elements across slides, shrink font to fit.`,

    markdown:`FORMAT: Markdown, Stripe/Vercel documentation standard. Renders in GitHub, Notion, Confluence.
STRUCTURE: H1 title (once) -- H2 sections -- H3 subsections. YAML frontmatter if appropriate. TOC with linked headings for 4+ sections. Horizontal rules between major sections.
TEXT: **Bold** key terms only. *Italic* for citations/tools. Backticks for code/filenames/commands. Blockquotes for callouts ("> **Note:**"). Ordered lists for sequential steps only. Max 2-level nesting.
TABLES: | Header | separated by |---|. Numbers right-aligned (---:), text left (:---). Max 6 columns. Empty = --. Caption above as bold text.
CODE: Language tag after triple backticks. Max 30 lines per block. One concept per block.
STANDARDS: Descriptive link text, alt text on images. No trailing whitespace, no tabs. Every claim backed by data or reasoning.`,

    html:`FORMAT: Semantic HTML5, Stripe/Linear quality. Production-ready, responsive, WCAG AA.
STRUCTURE: <header>, <nav>, <main>, <article>, <section>, <footer>. Single <h1>, never skip heading levels. Meta: charset UTF-8, viewport, description.
STYLING: BEM class names. Font: -apple-system, Inter, sans-serif. Body 16px, line-height 1.65, max-width 720px. CSS custom properties for colors (--color-primary: #1B2A4A, --color-accent: #4472C4, --color-text: #333, --color-bg-subtle: #F8F9FA). Responsive: CSS Grid, stack below 768px.
TABLES: <table>/<caption>/<thead>/<tbody> with <th scope>. Overflow-x wrapper for mobile. Header: bold, white on primary bg. Striped rows. Numeric: right-aligned, tabular-nums.
ACCESSIBILITY: Alt text on images. Focus styles (2px solid accent). 4.5:1 contrast ratio. ARIA labels. Skip-to-content link. Reduced-motion media query.
STANDARDS: Zero inline styles. 2-space indent. No deprecated elements. Print stylesheet. Lazy-load images.`,

    json:`FORMAT: Production-grade JSON, Stripe API / Bloomberg data feed quality. Parseable, self-documenting.
STRUCTURE: Root with "metadata" (schemaVersion, generatedAt ISO 8601, source, recordCount), "data", "summary". Max 4 levels deep. Arrays for same-type collections, objects for named properties.
NAMING: camelCase keys (totalRevenue, createdAt). No abbreviations. Boolean: is/has/can prefix. Arrays: plural nouns. Enums: lowercase_snake_case.
VALUES: Numbers raw numeric (never quoted). Missing: null (never 0, "", "N/A"). Dates: ISO 8601 with timezone. Currency: {amount, currency} object. Percentages: decimal (0.153 not 15.3%). IDs: string type.
STANDARDS: Valid JSON.parse(). 2-space indent. No trailing commas. Every array object has identical key sets. Sorted keys for deterministic output.`,

    csv:`FORMAT: Production CSV, direct import into Excel/pandas/R/BI tools. RFC 4180 compliant.
STRUCTURE: Row 1 headers in snake_case (revenue_usd, growth_rate_pct). One record per row, identical column count. Sort by primary axis. Column order: identifiers -- dimensions -- measures.
DATA: Double-quote fields with commas/newlines. Numbers raw (1234567.89, no separators). Dates ISO 8601 (YYYY-MM-DD). Currency numeric, noted in header. Percentages decimal. Missing: empty field (adjacent commas), never N/A/null/0.
STANDARDS: UTF-8, LF line endings. No trailing commas/whitespace/blank rows. Comment header: # Source, Date range, Generated, Column/Row count. Column definitions appendix.`,

    plaintext:`FORMAT: Professional plain text, top-tier firm memo standard. Any editor/terminal/email compatible.
STRUCTURE: Centered CAPS title, date (Month DD, YYYY), separator (======). Numbered headers (1.0, 1.1) with underline (------). "--- END OF DOCUMENT ---" at bottom.
LAYOUT: STRICT 80 char line width. Dash (-) for unordered lists, 2-space indent. Tables column-aligned with spaces, 4+ space gaps between columns, right-align numbers.
EMPHASIS: CAPS for headers/warnings only. *asterisks* for emphasis. [NOTE], [IMPORTANT], [ACTION REQUIRED] inline labels.
STANDARDS: ASCII only (-- not em-dash, " not smart quotes). Spaces only, no tabs. Never let table columns overlap or misalign.`,

    codeFile:`FORMAT: Production-ready code file, Google/Stripe/Vercel open-source quality. Runs without modification.
STRUCTURE: File header (filename, description, author, date, license SPDX). Imports grouped: stdlib -- third-party -- local. Constants -- Types -- Helpers -- Public API -- Entry point.
QUALITY: Canonical style guide (PEP 8, Airbnb, rustfmt). Type annotations on all signatures. Docstrings/JSDoc on public functions (@param, @returns, @throws, @example). Specific exception types with context. Input validation at public boundaries. Named constants, no magic numbers.
NAMING: Functions verb_noun (calculate_revenue). Classes PascalCase. Constants UPPER_SNAKE_CASE. Booleans is_/has_/can_ prefix. No single-letter names except loop counters.
STANDARDS: All imports resolvable. Logging over print. No hardcoded credentials. Parameterized queries. Performance notes where relevant.`
  };

  // Inject file output into contract
  if(fileOutput&&FILE_INST[fileOutput]){
    contractParts.push("File format: "+(FILE_OUTPUTS[fileOutput]?.l||fileOutput)+".\n\n"+FILE_INST[fileOutput]);

    // Industry-specific typography & color injection for styled file types
    ...
    if(styledFileTypes.includes(fileOutput)&&indStyle){
      ...
      contractParts.push(indStyle.label+" (ref: "+indStyle.ref+")\nFONTS: Headings: "+f.primary+". Body: "+f.secondary+". Data: "+f.data+". Mono: "+f.mono+".\nCOLORS: Primary: "+c.primary+". Secondary: "+c.secondary+". Accent: "+c.accent+". Text: "+c.text+". Muted: "+c.muted+". Header bg: "+c.headerBg+" / "+c.headerText+" text. Alt row: "+c.altRow+". Borders: "+c.border+". Negative: "+c.negative+". Positive: "+c.positive+".\nApply consistently across all pages, slides, tabs, tables, and charts.");
    }
  }

  // Finalize CONTRACT block
  if(contractParts.length>0){
    sec("contract",contractParts.join("\n\n"));
  }

  // ═══ BLOCK 4: QUALITY_GATE — verification checklist ═══
  const docFileTypes=["pdf","word","ppt","excel"];
  if(tier!=="fast"&&fileOutput&&docFileTypes.includes(fileOutput)){
    sec("quality_gate","Before delivering, verify every table and structured element:\n1. CELL INTEGRITY: No word split across lines -- abbreviate or widen if needed\n2. COLUMN FIT: No content wrapping -- redistribute widths\n3. PAGE FILL: No page <40% filled -- merge sections or extend\n4. NUMBER FORMAT: Identical formatting within each column (decimals, units, alignment)\n5. ABBREVIATIONS: Every abbreviation defined in glossary or footnote\nFix any failure. Standard: senior analyst at a top-tier investment bank preparing for a board presentation.");
  }

  // ═══ BLOCK 5: AVOID — anti-patterns ═══
  if(tier==="fast"){
    sec("avoid","No filler, hedging, or preamble. Start with substance, end with actionable specifics.");
  }else{
    sec("avoid","- No filler openers (\"Sure!\", \"Great question!\") -- begin with substance\n- No vague hedging (\"it depends\") without specifics\n- No generic background the audience already knows\n- No empty closing cliches -- end with actionable specifics\n- No restating the same point in different words");
  }

  // Token efficiency for non-flagship tiers
  if(tier==="balanced"){
    sec("efficiency","Balanced-tier model. Be thorough but concise. Combine related points. Maximum insight per paragraph.");
  }else if(tier==="fast"){
    sec("efficiency","Fast-tier model. Extremely concise. Bullets over paragraphs. Skip obvious context. Keep response short.");
  }
