...
const UI_FILE_OUTPUTS={
  pdf:{en:"PDF Document",no:"PDF-dokument"},word:{en:"Word / DOCX",no:"Word / DOCX"},excel:{en:"Excel / Spreadsheet",no:"Excel / Regneark"},ppt:{en:"PowerPoint / Slides",no:"PowerPoint / Lysbilder"},markdown:{en:"Markdown (.md)",no:"Markdown (.md)"},html:{en:"HTML Page",no:"HTML-side"},json:{en:"JSON / Structured Data",no:"JSON / Strukturerte data"},csv:{en:"CSV / Data File",no:"CSV / Datafil"},plaintext:{en:"Plain Text",no:"Ren tekst"},codeFile:{en:"Code File",no:"Kodefil"}
};
const UI_INCLUDES={
...
const EXAMPLES=[
  // FINANCE (5)
  {sector:"Finance",label:"DCF Valuation Model",topic:"Build a financial model comparing DCF valuations for a sustainable energy startup across bull/base/bear scenarios. Include sensitivities on capex, electricity prices, and battery cost learning curves. Target audience: equity research analysts.",task:"analysis",industry:"finance",output:"spreadsheet",model:"claude"},
  {sector:"Finance",label:"Short Seller Thesis",topic:"Write a short seller research report on a high-multiple SaaS company showing signs of revenue quality deterioration. Analyze deferred revenue trends, customer churn signals, and insider selling patterns. Present as a hedge fund investment memo.",task:"writing",industry:"finance",output:"document",model:"claude"},
  {sector:"Finance",label:"Sector Rotation Strategy",topic:"Analyze which equity sectors are best positioned for a late-cycle environment with persistent inflation. Compare energy, financials, healthcare, and utilities using relative valuation, earnings momentum, and macro sensitivity. Include specific ETF and stock picks.",task:"strategy",industry:"finance",output:"document",model:"general"},
  {sector:"Finance",label:"Earnings Call Analysis",topic:"Summarize and analyze the key takeaways from a major bank's Q4 earnings call. Focus on NII guidance, credit quality trends, capital return plans, and management tone vs. prior quarters. Highlight what the market might be missing.",task:"summarize",industry:"finance",output:"document",model:"chatgpt"},
  {sector:"Finance",label:"Risk Factor Decomposition",topic:"Build a multi-factor risk model for a Nordic equity portfolio. Decompose returns into market beta, size, value, momentum, and quality factors. Identify which factor exposures are intentional vs. accidental and recommend rebalancing actions.",task:"analysis",industry:"finance",output:"spreadsheet",model:"claude"},

  // TECH (5)
  {sector:"Tech",label:"System Architecture",topic:"Design a distributed task queue system that handles 10M daily jobs, with priorities, retries, and guaranteed-once execution. Explain trade-offs between Redis-based and database-backed queues.",task:"coding",industry:"tech",output:"document",model:"claude"},
  {sector:"Tech",label:"AI Product Roadmap",topic:"Propose a 6-month AI product roadmap for a B2B SaaS company: what early features unlock the most user value while being technically achievable? Include go-to-market angles and pricing strategy.",task:"strategy",industry:"tech",output:"proposal",model:"chatgpt"},
  {sector:"Tech",label:"Technical Due Diligence",topic:"Conduct a technical due diligence assessment for acquiring a 50-person startup. Evaluate their codebase quality, infrastructure scalability, technical debt, team capabilities, and integration complexity. What are the dealbreakers?",task:"review",industry:"tech",output:"document",model:"claude"},
  {sector:"Tech",label:"API Rate Limiter",topic:"Implement a distributed rate limiter using Redis that supports sliding window, token bucket, and fixed window algorithms. Must handle 100K requests/sec across multiple nodes. Include monitoring and graceful degradation.",task:"coding",industry:"tech",output:"code",model:"claude"},
  {sector:"Tech",label:"Cloud Cost Optimization",topic:"Analyze a company spending $2M/year on AWS. Identify the top 10 cost optimization opportunities across compute, storage, data transfer, and reserved capacity. Quantify expected savings for each recommendation.",task:"analysis",industry:"tech",output:"spreadsheet",model:"general"},

  // MARKETING (5)
  {sector:"Marketing",label:"Brand Positioning",topic:"Develop a positioning strategy for a D2C coffee brand competing against Nespresso and Blue Bottle. What's the emotional hook that justifies premium pricing? Include brand voice guidelines and messaging hierarchy.",task:"writing",industry:"marketing",output:"proposal",model:"chatgpt"},
  {sector:"Marketing",label:"Product Launch Tactics",topic:"Brainstorm 10 unconventional product launch tactics for a fintech app targeting Gen Z investors. Budget: $50K. The more creative and viral-worthy the better. Include expected CAC and reach estimates.",task:"brainstorm",industry:"marketing",output:"document",model:"general"},
  {sector:"Marketing",label:"Attribution Model",topic:"Design a multi-touch attribution model for an e-commerce company running paid search, social, email, and influencer campaigns. Compare last-click, linear, time-decay, and data-driven approaches. Recommend the best fit.",task:"analysis",industry:"marketing",output:"document",model:"claude"},
  {sector:"Marketing",label:"Content Strategy",topic:"Create a 90-day content marketing strategy for a B2B cybersecurity company targeting CISOs. Include content pillars, distribution channels, SEO keywords, and conversion funnel mapping. Define KPIs for each stage.",task:"strategy",industry:"marketing",output:"document",model:"chatgpt"},
  {sector:"Marketing",label:"Competitive Teardown",topic:"Conduct a competitive marketing teardown of the top 3 stock trading apps (Robinhood, Webull, eToro). Analyze their positioning, messaging, ad creative, social presence, and app store optimization. What gaps can a new entrant exploit?",task:"analysis",industry:"marketing",output:"document",model:"claude"},

  // LEGAL (5)
  {sector:"Legal",label:"EU AI Act Compliance",topic:"Analyze what EU AI Act compliance means for a US-based company providing computer vision APIs to insurance companies in Europe. What changes are needed to product, documentation, and governance?",task:"analysis",industry:"legal",output:"document",model:"general"},
  {sector:"Legal",label:"M&A Contract Strategy",topic:"Outline a contract negotiation strategy for acquiring a competitor. What key terms protect the buyer if post-close integration fails? Cover earn-outs, reps & warranties, indemnification caps, and MAC clauses.",task:"writing",industry:"legal",output:"document",model:"claude"},
...
function buildPrompt(p){
  ...
  // ROLE
  sec("role","You are "+( ROLE_DEEP[taskType]||"a helpful expert assistant")+".");

  // INDUSTRY CONTEXT
  if(industry!=="general"&&IND_CONTEXT[industry])sec("domain_expertise",IND_CONTEXT[industry]);

  // FIRM & ROLE CONTEXT
  if(selectedFirm&&SECTOR_FIRMS[industry]?.firms[selectedFirm]){
    const firmData=SECTOR_FIRMS[industry].firms[selectedFirm];
    let firmCtx="Adopt the working style, analytical frameworks, and communication standards of "+firmData.name+". Mirror their methodology, rigor, and deliverable quality.";
    if(selectedRole){firmCtx+="\n\nYou are operating in the role of a "+selectedRole+" at "+firmData.name+". Apply the specific expertise, priorities, and perspective this role demands. Frame your analysis and recommendations as this professional would.";}
    sec("firm_context",firmCtx);
  }

  // TASK + SMART ENHANCEMENT (adjusted by model tier)
  ...
  const tierSuffix=tier==="flagship"
    ?"\n\nYour response must be thorough, well-structured, and demonstrate expert-level depth. Deliver genuine professional value  -  not a surface-level overview."
    :tier==="balanced"
    ?"\n\nBe thorough but efficient. Cover all key points with clear structure. Prioritize actionable insights over exhaustive detail."
    :"\n\nBe concise and direct. Focus on the most important points. Use short paragraphs and clear structure. Skip preamble.";
  const sharpTopic=sharpenTopic(topic);
  const taskBlock=sharpTopic+(smartContext?"\n\n"+smartContext:"")+tierSuffix;
  sec("task",taskBlock);

  // ATTACHMENT CONTEXT
  if(hasAttachment){
    sec("attached_files","IMPORTANT: Files are attached alongside this prompt. Use the attached files as your PRIMARY data source.\n\n- Extract all relevant data, figures, tables, and context directly from the attached files\n- Ground your analysis, calculations, and conclusions in the actual data from the attachments\n- Reference specific data points, page numbers, or sections from the attachments when making claims\n- If the attachments contain tables or datasets, use those exact figures in your response -- do not fabricate or approximate\n- If the attachments are incomplete or missing data needed for the task, explicitly state what is missing and what assumptions you are making\n- Structure your output to complement the attachments -- do not repeat information verbatim, instead analyze, synthesize, and build upon it");
  }else{
    sec("source_guidance","No files are attached with this prompt. Generate all content, data, analysis, and examples based on your knowledge and the task description above.\n\n- Use realistic, well-researched figures and data points appropriate to the topic\n- When presenting financial figures, market data, or statistics, use plausible estimates and clearly label them as illustrative or estimated\n- Focus entirely on the stated goal -- provide maximum depth and specificity within your knowledge\n- If the task would benefit from specific source data you do not have, note what data sources would strengthen the analysis");
  }

  // CONTEXT
  const fmtLabels=(Array.isArray(format)?format:[format]).map(f=>FMTS[f]?.l||f).join(", ");
  const audienceEnhancement=enhancements.find(e=>e.type==="audience");
  const ctx=[];
  if(audience)ctx.push("Target audience: "+audience);
  else if(audienceEnhancement)ctx.push(audienceEnhancement.text);
  else ctx.push("Target audience: Knowledgeable professionals who expect depth and precision");
  ctx.push("Tone: "+tone+"  -  "+(TONE_INST[tone]||""));
  ctx.push("Writing style: "+(STYLES[style]?.l||style)+"  -  "+(STYLE_INST[style]||""));
  const fileLenSub=getLenSub(length,fileOutput);
  const defaultLenSub=LEN_SUB[length]||length;
  ctx.push("Length: "+fileLenSub+(fileLenSub!==defaultLenSub?" ("+defaultLenSub+")":""));
  ctx.push("Output format: "+fmtLabels);
  if(language&&language!=="English")ctx.push("Language: Respond entirely in "+language+". Use natural, native-quality "+language+" throughout.");
  sec("context",ctx.join("\n"));

  // OUTPUT TYPE
  if(output&&OUTPUT_INST[output])sec("output_type",OUTPUT_INST[output]);

  // METHODOLOGY (adapted to model tier)
  const methods=METHODOLOGY[taskType]||METHODOLOGY.writing;
  if(tier==="fast"){
    // For fast/light models: condensed methodology, max 3 steps
    const shortMethods=methods.slice(0,3);
    sec("methodology","Follow these steps:\n\n"+shortMethods.map((m,i)=>(i+1)+". "+m).join("\n"));
  }else{
    sec("methodology","Follow this structured approach:\n\n"+methods.map((m,i)=>(i+1)+". "+m).join("\n"));
  }

  // QUALITY (skip for fast tier to save tokens)
  if(tier!=="fast"){
    sec("quality_standards",QUALITY[taskType]||QUALITY.writing);
  }

  // RISK LEVEL
  const risk=riskLevel||"medium";
  if(risk==="high"){
    sec("risk_level","RISK LEVEL: HIGH. This task involves critical systems, sensitive data, or high-stakes decisions. Apply maximum caution:\n- Validate every assumption before acting on it\n- Add defensive error handling, input validation, and fallback paths\n- Flag any irreversible actions or destructive operations before proceeding\n- Consider edge cases, failure modes, and second-order consequences\n- When uncertain, state the uncertainty and present options rather than guessing\n- Include rollback procedures or undo paths where applicable");
  }else if(risk==="low"){
    sec("risk_level","RISK LEVEL: LOW. This is a routine, low-stakes task. Optimize for speed and directness:\n- Skip extensive caveats and edge case analysis unless directly relevant\n- Be concise and action-oriented\n- Prioritize the most likely scenario over exhaustive coverage");
  }

  // FORMAT DETAIL
  const fmtArr=Array.isArray(format)?format:[format];
  const FMT_INST={
    prose:"Use flowing, well-structured paragraphs with clear topic sentences and logical transitions. Use subheadings to organize major sections.",
    bullets:"Use concise bullet points grouped under clear section headers. Each bullet should be a complete thought (1-2 sentences). Lead with the most important information in each bullet.",
    numbered:"Present key sequences as clearly numbered steps. Start each step with a bold action verb. Include brief explanations under each step when needed.",
    table:"Organize comparative or multi-variable information into well-labeled tables with clear column headers. BEFORE CREATING ANY TABLE: (1) List all column headers and the widest value in each column. (2) Verify total width fits the output format -- if not, abbreviate headers (Rev., Gr., Mgn., FY25E) or split into multiple tables. (3) Pre-select abbreviations and define them in a footnote. TABLE RULES: (1) Every cell displays complete content -- zero truncation, overflow, or mid-word breaking. (2) Column widths auto-adjust to the longest value plus generous padding. (3) Numbers are right-aligned and never wrap. (4) Maximum 2 lines per cell. (5) Consistent number formatting per column (commas for thousands, 1 decimal for %, negatives in parentheses). (6) Financial tables use short year labels (FY24A, FY25E), $ on first/total rows only, right-aligned values. (7) NEVER display N/A, n/a, NA, or None in any cell -- use em dash (--) or leave blank; if an entire column would be N/A, remove the column. (8) Zero values: show as centered em dash (--), NEVER as 0, 0.0, $0, or 0%. (9) Unit qualifiers (NOK '000, USD millions) go in a small subtitle below the table title, never inline at the same font size. (10) Uniform cell padding and row heights throughout -- no inconsistent spacing between rows or columns. Add a narrative summary before and after each table highlighting key takeaways.",
    qa:"Structure appropriate sections as specific questions followed by thorough answers. Order from foundational to advanced. Make each Q&A pair self-contained.",
    headers:"Use a clear hierarchy of section headers (H2 for major sections, H3 for subsections). Every section should be scannable independently. Include a table of contents for longer responses.",
    tldr:"Begin with a TL;DR section (3-5 sentences capturing the absolute essence), then follow with the full detailed response. The TL;DR must be useful on its own.",
    framework:"Structure your response around a recognized framework or model relevant to the domain. Name the framework explicitly, explain why it applies, and map your analysis to its components.",
    flowchart:"Include text-based decision trees or flowcharts (using arrows and branching) to illustrate processes, decisions, or workflows. Label each decision point and outcome clearly.",
    before_after:"Where applicable, use Before / After comparisons to illustrate transformations, improvements, or changes. Show concrete examples of the current state vs. the recommended state.",
    pros_cons:"For any recommendation or option, include a structured Pros & Cons analysis. Be specific and quantify where possible.",
    scorecard:"Evaluate options using a structured scorecard or rating matrix. Define clear criteria, use a consistent scale (e.g., 1-5 or H/M/L), and include weighted totals if appropriate. BEFORE BUILDING: count columns and verify they fit the output width -- use H/M/L instead of High/Medium/Low, abbreviate criteria names if needed. MATRIX RULES: Equal-width columns for rating columns, wider column for labels/criteria. Every cell fully visible with no overlap or truncation. Center-align ratings, left-align criteria names. Consistent formatting across all cells in each column.",
    annotated:"Include annotated examples that walk through real or realistic scenarios step by step, with inline commentary explaining why each element matters and what to watch for."
  };
  const fmtL=[];
  if(fmtArr.length===0)fmtL.push("Use the most appropriate format for the content.");
  else fmtL.push("Combine these formatting approaches throughout your response as appropriate:\n\n"+fmtArr.map((f,i)=>(i+1)+". **"+(FMTS[f]?.l||f)+"**: "+(FMT_INST[f]||"")).join("\n"));
  const lenG={Brief:"~200-400 words. Only highest-impact information.",Medium:"~500-900 words. Cover all major aspects with supporting detail.",Detailed:"~1000-1800 words. Thorough explanations, examples, and nuance.",Comprehensive:"~2000+ words. Exhaustive reference-grade. Every dimension with depth, examples, evidence."};
  fmtL.push(lenG[length]||lenG.Medium);
  sec("formatting",fmtL.join("\n\n"));

  // FILE OUTPUT FORMAT — Professional-grade formatting standards per file type
  const FILE_INST={
    pdf:`Design as a Goldman Sachs / J.P. Morgan quality PDF document suitable for C-suite presentation. Every element must be precisely formatted with zero visual defects.

DOCUMENT STRUCTURE:
- Title page: document title (bold, 28pt Helvetica Neue or Arial), subtitle (16pt, regular weight, 8pt below title), author/team name, date (format: Month DD, YYYY), classification ("Confidential" or "For Discussion Purposes Only"), organization logo placeholder top-right
- Table of contents for any document over 3 pages, with right-aligned page numbers connected by dot leaders
- Numbered sections (1.0, 1.1, 1.2) with consistent heading hierarchy throughout
- Page numbers centered in footer (9pt), document title left-aligned in header (8pt, gray #999)
- Hard page breaks before every major section (never start a major section mid-page)
- Footer: thin 0.5pt navy rule above footer content

TYPOGRAPHY & LAYOUT:
- Body text: 10.5pt Arial or Helvetica, #333333, 1.35 line spacing, left-aligned (not justified -- prevents uneven word spacing)
- H1: 20pt Arial Bold, #1B2A4A, 24pt space before, 8pt space after, with 1pt navy rule below spanning full text width
- H2: 14pt Arial Bold, #1B2A4A, 18pt space before, 6pt space after
- H3: 11pt Arial Bold, #333333, 12pt space before, 4pt space after
- Margins: 0.75 inch top/bottom, 1 inch left/right (standard IB layout)
- Paragraph spacing: 6pt after each paragraph, never use double line breaks
- Key findings: light blue (#EBF5FB) background callout box with 10px internal padding, 1pt left border in navy, 10pt body text
- Bullet points: 9.5pt, 6pt spacing between bullets, em dash style (--) not round dots

TABLE FORMATTING (CRITICAL -- this determines document quality):
- Table width: 100% of text area, never overflow margins
- Header row: 10pt Arial Bold, white (#FFFFFF) text on navy (#1B2A4A) background, 10px cell padding top/bottom, 12px left/right padding
- Sub-header rows (if needed): 10pt Arial Bold on medium gray (#E8E8E8)
- Data rows: 9.5pt Arial Regular, 8px cell padding top/bottom, 12px left/right padding
- Alternating row shading: white (#FFFFFF) and very light gray (#F8F9FA) -- subtle, never strong contrast
- Borders: 0.5pt solid #D0D5DD on all cell borders, 1pt solid #1B2A4A on header bottom border
- Column alignment: text LEFT-aligned, numbers RIGHT-aligned, headers match their column data alignment
- Number formatting: right-align all numbers, use commas for thousands (1,234,567), 1 decimal for percentages (12.3%), 2 decimals for currencies ($1,234.56), negative values in parentheses with no minus sign: ($12.4M)
- Column widths: calculate proportional to longest content in each column PLUS 24px padding -- NEVER allow text truncation, wrapping to more than 2 lines, or column overlap
- Table title: 10pt Bold above table with 4pt space below, format "Table N: [Descriptive Title]"
- Table notes/source: 8pt Italic below table with 4pt space above, format "Source: [source]. Note: [any caveats]"
- Minimum column width: 60px -- no column should be narrower
- If table exceeds page width: switch to landscape orientation for that page, or split into two tables
- NEVER place two tables adjacent horizontally -- always stack vertically with 16pt space between

EMPTY VALUE & UNIT HANDLING (CRITICAL -- eliminates visual clutter):
- NEVER display "N/A", "n/a", "NA", "None", or any not-available placeholder in table cells -- leave the cell blank or use a single centered em dash (--)
- Zero values: display as a centered em dash (--), NEVER show "0", "0.0", "$0", or "0%" -- zeros clutter tables and add no information
- If an entire column or row would be blank/dashes, REMOVE that column or row entirely from the table
- If a table section (e.g., "Baseline" column) has no real data yet, omit the column rather than filling it with N/A or dashes
- Unit qualifiers (e.g., "(NOK '000)", "(USD millions)", "(in thousands)"): place as a small 7.5pt italic annotation right-aligned in the table title row or as a subtitle line directly below the table title in 8pt italic gray (#999) -- NEVER embed units in the main table title at the same font size. Format: "Table N: Financial Projections" on first line, "(NOK '000)" on second line in 8pt italic gray, right-aligned to the table width
- Currency/unit labels: show on the first data row and total rows only, omit from middle rows to reduce visual noise

SPACING DISCIPLINE (CRITICAL -- prevents uneven layouts):
- Consistent cell padding: exactly 8px top/bottom, 12px left/right for ALL data cells -- no exceptions
- Row height: uniform 20px for all data rows within a table section, 24px for header rows
- Between table title and header row: exactly 4pt, never more
- Between last data row and source line: exactly 4pt
- Between consecutive tables: exactly 20pt vertical gap
- Between section heading and first element below: exactly 8pt
- Between last element and next section heading: exactly 16pt
- No extra blank lines or double-spacing within tables or between table elements
- Column gaps: minimum 12px visual separation between column content -- no touching or overlapping text

FIGURES & CHARTS:
- Figure title above: "Figure N: [Insight statement, not just topic]" in 10pt Bold
- Charts: clean axes with 8pt labels, no gridlines unless essential (then light gray dashed 0.25pt)
- Color palette: navy (#1B2A4A), steel blue (#4472C4), teal (#0891B2), medium gray (#A5A5A5), light gray (#D9D9D9) -- max 5 colors per chart
- Data labels directly on bars/lines in 8pt, minimize legend usage
- Source line below: 8pt Italic gray, 4pt above
- White space: 12pt above figure title, 16pt below source line

TABLE PLANNING PHASE (MANDATORY -- do this BEFORE writing any table):
For EVERY table in the document, you MUST complete these planning steps before generating the table content:

Step 1 -- INVENTORY: List every column header and the widest value that will appear in that column. Include units, signs, and formatting (e.g., "($1,234.5M)" is 12 characters, "FY2025E" is 7 characters).

Step 2 -- FIT TEST: Count total character width across all columns. A portrait PDF fits approximately 90-100 characters across (at 9.5pt). If your total exceeds this:
  (a) Shorten headers using standard abbreviations: "Rev." for Revenue, "Gr." for Growth, "Mgn." for Margin, "FY25E" instead of "FY2025E", "EV/Rev" not "Enterprise Value / Revenue"
  (b) If still too wide, remove lower-priority columns and move that data to a separate table
  (c) If the data genuinely needs 8+ columns (e.g., multi-year projections), switch to landscape orientation for that page
  (d) NEVER squeeze columns to the point where any value wraps or breaks

Step 3 -- ABBREVIATION GLOSSARY: Collect every abbreviation used across all tables. Place a single consolidated footnote section at the end of the document: "Abbreviations: Rev. = Revenue; Gr. = Growth; E = Estimated; A = Actual; ..." This is defined ONCE, not repeated per table.

Step 4 -- STRUCTURE DECISION: For each table, decide:
  - Orientation: portrait (default) or landscape (if 7+ data columns)
  - Column count: maximum 7 in portrait, 10 in landscape
  - Font size: 9.5pt default, 8.5pt minimum for wide tables
  - Whether to split into multiple tables (preferred over cramming)

Step 5 -- CONTENT-SPECIFIC ADAPTATION: Design each table for the specific data it contains:
  - Financial projections: use short fiscal year labels (FY24A, FY25E, FY26E), right-align all numbers, $ sign on first and total rows only
  - Comparable company tables: company names in first column (widest), multiples in narrow right-aligned columns with 1 decimal + "x"
  - Risk matrices: use "H/M/L" or color indicators instead of spelling out "High/Medium/Low" in cells
  - DCF tables: label column left, all value columns right-aligned, use consistent $M or $B notation
  - Sensitivity tables: highlight base case with bold border, keep inputs on axes clearly labeled

CONTENT-FITTING RULES:
- Every cell displays COMPLETE content -- zero truncation, zero overflow, zero mid-word breaks
- Column widths auto-adjust to the widest value plus 24px padding per side
- Numbers NEVER wrap to a second line
- Maximum 2 lines per cell -- if more needed, shorten wording or footnote
- Row heights uniform within each section
- Minimum 20pt vertical gap between tables
- Matrix layouts: equal-width rating columns, wider label column, centered content

PAGE FLOW RULES (prevents blank pages):
- Before placing a table, verify it fits on the current page. If less than 30% of the page remains, start the table on the next page
- NEVER place a table title on one page and the table body on the next
- NEVER let a table split with fewer than 2 data rows on either side of a page break
- If a section has a short paragraph followed by a table, keep them together on the same page
- Source lines must appear directly below their table on the same page, never orphaned

CRITICAL DO-NOT RULES:
- NEVER allow a word to break mid-syllable across lines (no hyphenation in table cells)
- NEVER overlap text, columns, rows, or visual elements
- NEVER truncate numbers, labels, or headers
- NEVER use bright colors, decorative fonts, clip art, or stock photography
- NEVER leave orphan/widow lines
- NEVER hardcode line breaks inside table cells to force-fit content
- NEVER use font sizes below 8pt for any element
- NEVER let adjacent columns have less than 12px visual gap
- Every number formatted consistently within its column
- Every table and figure must have both a title above and source below`,

    word:`Design as a professional Word/DOCX document at the quality standard of a Big 4 consulting deliverable or top-tier law firm memo. Zero formatting inconsistencies allowed.

DOCUMENT STRUCTURE:
- Title page: document title (Calibri 26pt Bold, #1B2A4A), subtitle (14pt Regular, #555), author/team, date (Month DD, YYYY), version number, confidentiality marking, thin navy rule separating title from metadata
- Auto-generated Table of Contents using Word Heading styles (Heading 1, 2, 3) with dot leaders and right-aligned page numbers
- Section numbering (1.0, 1.1, 1.2) using Word built-in multilevel list linked to Heading styles
- Header: document title left (8pt Calibri, gray #999), thin 0.5pt rule below
- Footer: "Page X of Y" centered (8pt Calibri), confidentiality notice right-aligned (8pt, gray)
- Hard page breaks before every Heading 1 section

STYLES & TYPOGRAPHY:
- Body: Calibri 11pt, #333333, 1.25 line spacing, left-aligned (not justified), 6pt space after paragraphs
- Heading 1: Calibri 18pt Bold, #1B2A4A, 24pt space before, 8pt space after, 1pt navy bottom border spanning full width
- Heading 2: Calibri 14pt Bold, #1B2A4A, 16pt space before, 6pt space after
- Heading 3: Calibri 11.5pt Bold, #333333, 12pt space before, 4pt space after
- Bullet lists: Calibri 10.5pt, em dash (--) style, 4pt space between items, 12pt left indent, 6pt hanging indent
- Numbered lists: Arabic numerals, same spacing as bullets, bold number with regular text
- Captions: Calibri 9pt Italic, #666, 4pt above/below
- Key findings callout: light blue (#EBF5FB) shaded paragraph, 1pt navy left border, 10pt internal padding all sides

TABLE FORMATTING (CRITICAL):
- Table width: 100% of text area between margins
- Header row: Calibri 10pt Bold, white text on navy (#1B2A4A), 8px cell padding all sides, "Repeat as header row" enabled for multi-page tables
- Data rows: Calibri 10pt Regular, 6px cell padding top/bottom, 8px left/right
- Alternating row banding: white and #F8F9FA (very subtle)
- Borders: 0.5pt #D0D5DD all cells, 1.5pt navy below header row
- Numbers: RIGHT-aligned, commas for thousands, 1 decimal for percentages, 2 for currencies, negatives in parentheses ($12.4M)
- Text columns: LEFT-aligned, headers match column alignment
- Column widths: auto-fit to content with minimum 60px per column, NEVER truncate or overlap
- Table title above: "Table N: [Title]" in Calibri 10pt Bold, 4pt below
- Table source below: "Source: [source]" in Calibri 8pt Italic, 4pt above
- Tables MUST NOT break mid-row across pages -- keep rows together, allow page break only between rows
- If table is wider than margins: switch to landscape section break for that page only

TABLE PLANNING PHASE (MANDATORY -- do this BEFORE writing any table):
Step 1 -- INVENTORY: List every column header and the widest value expected. Include formatting characters (parentheses, $, %, x).
Step 2 -- FIT TEST: A portrait Word doc fits ~95-105 characters across at 10pt Calibri. If total column width exceeds this: (a) abbreviate headers (Rev., Gr., Mgn., FY25E), (b) remove low-priority columns to a separate table, (c) switch to landscape section break.
Step 3 -- ABBREVIATION GLOSSARY: Define all abbreviations once at the end of the document.
Step 4 -- CONTENT-SPECIFIC ADAPTATION: Design table structure for the actual data -- financial projections use short year labels (FY24A, FY25E), comps tables put company names in the widest column, risk matrices use H/M/L not spelled out.

CONTENT-FITTING RULES:
- Every cell shows COMPLETE text -- no truncation, no overflow, no mid-word breaks
- Auto-fit column widths to longest value plus 16px padding per side
- Numbers NEVER wrap -- widen the column instead
- Maximum 2 lines per data cell
- Uniform row heights within each table section
- Minimum 12px visual gap between adjacent column content
- Tables wider than margins: landscape section break or split vertically
- Matrices/scorecards: equal-width rating columns, wider label column, centered cell content

PAGE FLOW RULES:
- Verify each table fits on the current page before placing it -- if less than 30% remains, start on next page
- Table title and table body must be on the same page
- Source lines must appear directly below their table, never orphaned on next page
- Never split a table with fewer than 2 data rows on either side of a page break

CRITICAL DO-NOT RULES:
- NEVER allow word-breaking (hyphenation) inside table cells
- NEVER overlap table columns or truncate cell content
- NEVER use manual formatting (direct font changes) -- everything must use defined Styles
- NEVER leave orphan/widow lines
- NEVER mix number formats within a column
- NEVER use font below 8pt for any element
- NEVER let two tables touch without at least 16pt vertical space between them
- All tables and figures must have title above and source below`,

    excel:`Design as an investment-bank-grade Excel workbook at the standard of Goldman Sachs TMT / J.P. Morgan M&A advisory. This workbook must be ready to present to a board of directors or IPO steering committee. Zero tolerance for formatting inconsistencies.

WORKBOOK STRUCTURE:
- Tab naming: numbered with clear labels ("1. Summary", "2. Revenue Build", "3. DCF Valuation", "4. Comps", "5. Sensitivity", "6. Sources & Assumptions")
- First tab: Executive Summary with key metrics dashboard, investment thesis, and hyperlinked navigation to all other tabs
- Each tab: ONE analytical purpose only -- never mix unrelated analyses on the same sheet
- Tab order: narrative flow from summary > analysis > valuation > sensitivity > appendix

CELL & LAYOUT FORMATTING (CRITICAL -- defines workbook quality):
- Row 1: Company name, bold 12pt Calibri, navy (#1B2A4A), merged across data width
- Row 2: Report title, bold 10pt, #333333
- Row 3: Date (Month DD, YYYY), analyst name, classification -- 9pt, #666
- Row 4: Empty spacer row (height 6px)
- Row 5+: Data begins with header row

- Header rows: 10pt Calibri Bold, white (#FFF) text on navy (#1B2A4A) background, center-aligned, 22px row height, bottom border 1.5pt navy
- Sub-header rows: 10pt Calibri Bold, #1B2A4A text on light gray (#E8E8E8) background
- Data rows: 9.5pt Calibri Regular, 18px row height, alternating white/#F8F9FA shading
- Category/label column: LEFT-aligned, bold for section totals, regular for line items, 4px left indent
- All number columns: RIGHT-aligned, 8px right padding
- Column widths: auto-fit to widest content + 20px padding minimum -- NEVER allow ### overflow or truncation
- Minimum column width: 55px for numbers, 100px for text labels
- Borders: 0.5pt #D0D5DD horizontal lines between rows, 1pt navy below headers, light vertical borders only on outer edges
- Gridlines: HIDE default gridlines -- use explicit borders only

NUMBER FORMATTING (IB standard -- non-negotiable):
- Revenue/costs in millions: one decimal, dollar sign on first and last row only: $1,234.5 (first row), 1,234.5 (middle rows), $1,234.5 (total row)
- Percentages: one decimal with % symbol, right-aligned: 12.3%
- Multiples: one decimal with "x" suffix: 8.4x
- Per-share values: two decimals: $45.67
- Large numbers: always use commas: 1,234,567
- Negative values: parentheses, red font (#CC0000), no minus sign: ($12.4M)
- Zero values: show as dash (--) centered, NEVER show "0", "0.0", "$0", or "0%" -- zeros are visual clutter
- N/A values: NEVER display "N/A", "n/a", "NA", or "None" -- use em dash (--) or leave blank. If an entire column would be N/A, remove the column entirely
- Dates: MMM-YY for column headers (Jan-26, Feb-26), full date in title block
- Growth rates: show with + prefix for positive: +8.3%, negative in red parentheses: (4.2%)
- Unit qualifiers in titles (e.g., "NOK '000", "USD millions"): place in cell B3 or C3 as 8pt italic gray (#999), right-aligned -- NEVER in the same cell as the main title at the same font size

INPUT VS CALCULATION DISTINCTION:
- Input/assumption cells: light blue (#DCE6F1) background -- user can modify these
- Calculated cells: no special background -- formulas only, never hardcoded
- Linked cells (from other tabs): green font (#006100) to show data flows from elsewhere
- Hard-coded overrides: orange font (#E26B0A) with comment explaining why

FORMULAS & CALCULATIONS:
- Every calculated cell MUST use formulas -- zero hardcoded values in calculated areas
- Named ranges for all key assumptions (growth_rate, discount_rate, tax_rate, terminal_multiple)
- Formula audit trail: add a comment on complex formulas explaining the logic
- Scenario toggles: Bull / Base / Bear using data validation dropdown linked to named ranges
- IFERROR wrappers on all division operations and lookups
- Circular reference protection: use iterative calculation settings note if needed

ANALYTICAL COMPONENTS:
- Revenue bridge / waterfall: show sequential drivers (price, volume, mix, FX) with subtotals
- Sensitivity tables: two-variable data tables (e.g., growth rate vs. discount rate) with current case highlighted in bold border
- Comparable company analysis: show company name, ticker, market cap, EV, revenue, EBITDA, margins, and trading multiples in standard comps table format
- DCF components: explicit WACC build, unlevered FCF projection, terminal value (both perpetuity growth and exit multiple), enterprise value bridge to equity value
- Returns analysis: IRR and MOIC calculations with entry/exit assumptions

CHARTS (describe precisely for each):
- Chart type: specify (clustered bar, stacked bar, line, combo dual-axis, waterfall)
- Dimensions: 500x300px approximate, positioned below or right of source data
- Axis labels: 8pt Calibri, #666, no gridlines unless essential (then 0.25pt dashed #E0E0E0)
- Data labels: 8pt, directly on bars/lines, formatted to match number format of source data
- Colors: navy (#1B2A4A) primary, steel blue (#4472C4) secondary, teal (#0891B2) tertiary, gray (#A5A5A5) for benchmarks
- Title: 10pt Bold, states the analytical conclusion ("Revenue grew 23% YoY driven by pricing"), NOT just the topic

PROFESSIONAL STANDARDS:
- Freeze panes: lock Row 5 (header) and Column A (labels) on every data tab
- Print area: defined on every tab, landscape orientation for wide tables, fit to 1 page wide
- Sources & Assumptions tab: document every data source, retrieval date, and key assumption with rationale
- Data validation dropdowns for scenario selectors and input fields
- Conditional formatting: use for KPI status (green #00B050 / yellow #FFC000 / red #FF0000), data bars for relative comparison
- Protection: suggest which cells to lock (all except blue input cells)

TABLE PLANNING PHASE (MANDATORY -- do this BEFORE writing any data table):
Step 1 -- INVENTORY: For each tab, list every column header and its widest expected value including formatting ($, commas, parentheses, x suffix, % sign).
Step 2 -- FIT TEST: A landscape Excel printout fits ~130-150 characters at 9.5pt. If columns exceed this: (a) abbreviate headers, (b) split into multiple tabs, (c) use narrower number formats ($1.2B instead of $1,234.5M).
Step 3 -- CONTENT-SPECIFIC ADAPTATION: Financial models use FY24A/FY25E short labels, comps tables show ticker symbols not full names in header, sensitivity tables use 2-variable grid with clear axis labels, DCF models separate assumptions from calculations with visual dividers.

CONTENT-FITTING RULES:
- Every cell displays COMPLETE content -- no ### symbols, no cut-off text, no hidden overflow
- Auto-fit all columns to widest content plus 20px padding
- Numbers NEVER wrap -- widen immediately if needed
- Row heights: uniform within sections (headers 22px, data 18px)
- Merged cells: ONLY in title block (rows 1-3), NEVER in data ranges
- Between separate tables on same sheet: minimum 3 empty rows as separator

SPACING DISCIPLINE (prevents sloppy layouts):
- Row heights: exactly 22px for headers, exactly 18px for data rows -- no variation within a section
- Column padding: exactly 8px right padding for numbers, 6px left padding for labels -- uniform throughout
- Section separator rows: exactly 6px height, no content, light gray (#F2F2F2) background -- creates clean visual breaks between table sections
- Between title block (rows 1-3) and data: exactly 1 empty spacer row (row 4, height 6px)
- Between separate data sections on same tab: 1 spacer row (6px height, light gray fill)
- Between separate tables on same sheet: minimum 3 empty rows as separator
- Column gaps: minimum 4px visual separation between adjacent column content -- no touching text
- Consistent indentation: sub-items indent 12px from parent category labels, sub-sub-items indent 24px

PROFESSIONAL COLORING (IB standard):
- Section total rows: bold text, 1pt navy top border, light navy (#E8EEF4) background -- clearly distinguishes totals from line items
- Grand total rows: bold text, double navy top border (2pt), light navy (#E8EEF4) background
- Negative contribution / loss rows: red font (#CC0000) for values, normal font for labels
- Positive milestones or targets: dark green font (#006100) for positive KPI values
- Key metric rows (revenue, net income, EBITDA): bold label text to draw the eye
- Status indicators: conditional formatting with green (#00B050) / amber (#FFC000) / red (#FF0000) -- use sparingly, only on KPI or status columns

CRITICAL DO-NOT RULES:
- NEVER show ### in any cell -- auto-fit or manually widen columns
- NEVER allow word-breaking or hyphenation in any cell
- NEVER mix number formats within a column
- NEVER leave blank rows in the middle of a data table (use spacer formatting instead)
- NEVER use merged cells in data ranges (breaks sorting and formulas) -- merge only in title block
- NEVER hardcode a value that should be calculated
- NEVER use more than 5 colors in a single chart
- NEVER display "N/A" or "None" in any cell -- use em dash (--) or remove the column/row if all values are unavailable
- NEVER show raw zero (0) in financial cells -- use em dash (--) centered
- Every number must trace to a source, formula, or clearly marked assumption`,

    ppt:`Design as a Goldman Sachs / McKinsey quality executive presentation (pitchbook standard). Every slide must look like it was reviewed by a Managing Director before a $5B+ deal meeting. Zero tolerance for misaligned elements, inconsistent formatting, or visual clutter.

SLIDE STRUCTURE:
- Title slide: presentation title (28pt Arial Bold, white on navy #1B2A4A full-bleed background), subtitle (16pt Regular, white 70% opacity), presenter name and title, date (Month YYYY), confidentiality line bottom ("Goldman Sachs Confidential" or similar placeholder), logo placeholder top-right
- Agenda slide: numbered list (max 6 items), each item hyperlinked to its section divider
- Section divider slides: navy (#1B2A4A) full background, section number (72pt, white 20% opacity) top-left, section title (28pt Bold, white) center-left, one-line section description (14pt, white 60% opacity) below
- Content slides: Assertion-Evidence format MANDATORY -- slide title IS the key insight/conclusion (not a topic label), body PROVES the assertion with data
- Summary slide: "Key Takeaways" with 3-5 bold assertions, then "Recommended Next Steps" with numbered actions, owners, and timelines
- Appendix slides: labeled "Appendix [letter]:" with supporting detail

MASTER LAYOUT (every content slide):
- Slide dimensions: 16:9 widescreen (13.33" x 7.5")
- Title area: top 1.1 inches, left-aligned at 0.5" from left edge
- Title text: 20pt Arial Bold, #1B2A4A, max 2 lines
- Content area: 0.5" left margin, 0.5" right margin, starts at 1.3" from top, ends at 6.8" from top
- Footer bar: thin 0.5pt navy rule at 7.0" from top, below it: source text left (8pt Italic, #999), slide number right (8pt, #999), company name center (8pt, #999)
- Logo placeholder: top-right corner, 0.3" from edges, max 0.6" height

TYPOGRAPHY:
- Title: 20pt Arial Bold, #1B2A4A, left-aligned, max 2 lines -- if it doesn't fit in 2 lines, sharpen the wording
- Body text: 14pt Arial Regular, #333333, left-aligned, 1.2 line spacing
- Bullet points: 13pt, em dash (--) style, 4pt space between bullets, max 5 bullets per slide, max 2 lines per bullet
- Bullet style: telegraphic (action verb + key metric + insight) -- NEVER full sentences
- Sub-bullets: 11pt, 16pt left indent from parent, circle marker, lighter gray #555
- Callout numbers: 36pt Bold for hero metrics (e.g., "$4.2B" or "+23%"), navy or accent color, with 11pt descriptor below
- Small print: 8pt minimum for source lines and footnotes

TABLE FORMATTING ON SLIDES (CRITICAL):
- Maximum visible: 7 rows x 6 columns -- if more data, split across slides or move to appendix
- Header row: 10pt Arial Bold, white on navy (#1B2A4A), 6px cell padding
- Data rows: 9.5pt Arial Regular, 5px cell padding, alternating white/#F8F9FA
- Borders: 0.5pt #D0D5DD horizontal only (no vertical borders for cleaner look), 1.5pt navy below header
- Numbers: RIGHT-aligned, same IB formatting (commas, parentheses for negatives, 1 decimal %)
- Text: LEFT-aligned, 4px left padding
- Column widths: proportional to content, NEVER allow overlap or truncation -- if table is too wide, reduce columns or use abbreviations with a footnote
- Total/summary row: bold text, 1pt top border, light navy (#E8EEF4) background
- Table must be horizontally centered on slide with equal margins left and right

DATA VISUALIZATION:
- Charts ALWAYS preferred over tables for trends, comparisons, and compositions
- Chart size: fill content area (approximately 11" wide x 4.5" tall for full-width charts)
- Chart title: 11pt Bold, states the CONCLUSION not the topic ("Revenue grew 23% driven by pricing power" NOT "Revenue Overview")
- Axis labels: 8pt Arial, #666, clean readable values (use $M, $B abbreviations)
- Gridlines: none or very light (0.25pt dashed #E8E8E8, horizontal only)
- Data labels: 8pt, directly on bars/points, formatted consistently, no overlapping
- Colors: navy (#1B2A4A) primary, steel blue (#4472C4) secondary, teal (#0891B2) tertiary, light gray (#D9D9D9) for context/benchmark, red (#DC2626) for emphasis only -- max 5 colors
- Legend: only if 3+ data series, placed below chart in 8pt, horizontal layout
- Source line: 8pt Italic #999, bottom-left of chart, format "Source: [name], [date]"

SPEAKER NOTES (every content slide):
- Opening line: the key message in one sentence
- 3-4 supporting data points with specific numbers
- Anticipated pushback/question and your response
- Transition phrase to next slide
- Format: bullet points, 11pt, complete sentences allowed in notes

TABLE PLANNING PHASE (MANDATORY -- do this BEFORE creating any slide table):
Step 1 -- INVENTORY: List every column and the widest value. A 16:9 slide fits approximately 70-80 characters across at 9.5pt in the content area.
Step 2 -- FIT TEST: If total column width exceeds slide width: (a) abbreviate all headers aggressively (Rev., EBITDA, EV/Rev, FY25E), (b) reduce to max 6 columns, (c) move excess columns to a second slide or appendix. NEVER cram -- split instead.
Step 3 -- CONTENT-SPECIFIC ADAPTATION: Financial summary slides show only 3-5 key metrics with large callout numbers, comps tables max 5 companies x 5 metrics, DCF summary shows only enterprise value bridge (not full model), risk matrices use 3x3 grid with color coding instead of text.
Step 4 -- SLIDE BUDGET: Each content slide gets ONE table OR ONE chart, never both. If you need both, use two slides.

CONTENT-FITTING RULES:
- Every table cell shows COMPLETE text -- no truncation, no overflow, no word-breaking
- Column widths proportional to content with 8px padding per side
- Slide tables: max 7 rows x 6 columns -- split or appendix if more
- Numbers NEVER wrap -- widen column or abbreviate units ($M, $B)
- Bullet text: 1-2 lines max, split or shorten if longer
- Slide titles: exactly 1-2 lines, sharpen wording instead of shrinking font
- All elements have at least 0.3" margin from slide edges and each other
- Charts and tables horizontally centered with equal left/right margins
- No element overlaps another or extends beyond slide boundaries

CRITICAL DO-NOT RULES:
- NEVER allow any word to break mid-syllable in tables, bullets, or titles
- NEVER misalign elements across slides -- titles, content areas, and footers must be at identical positions on every slide
- NEVER exceed 5 bullets or 7 table rows on a single slide
- NEVER use animations, transitions, sound effects, or decorative graphics
- NEVER use clip art, stock photos, or WordArt
- NEVER let data labels overlap each other or chart elements
- NEVER use font below 8pt for any visible element
- NEVER place text directly against a slide edge without margin
- NEVER shrink font to fit content -- instead shorten the content
- NEVER present a number without a source (in footer or speaker notes)
- Every chart, table, and data point must have a source attribution`,

    markdown:`Design as clean, professional Markdown optimized for rendering in GitHub, Notion, Confluence, or any standard Markdown viewer. Quality standard: technical documentation at Stripe or Vercel level.

DOCUMENT STRUCTURE:
- H1 (#) for document title only -- used exactly once at the top
- H2 (##) for major sections, H3 (###) for subsections, H4 (####) sparingly
- YAML frontmatter block: title, date, author, status, version (if appropriate for platform)
- Table of contents at top using linked headings: "- [Section Name](#section-name)" for documents with 4+ sections
...
TEXT FORMATTING:
- **Bold** for key terms, findings, and emphasis only -- never bold entire sentences
- *Italic* for citations, definitions, tool names, and secondary emphasis
- Inline code backticks for: file names, function names, CLI commands, variables, API endpoints, config keys
- Blockquotes (>) for: key findings, important callouts, or quoted material -- prefix with "> **Note:**" or "> **Important:**" for clarity
- Ordered lists (1. 2. 3.) for sequential steps only
- Unordered lists (- ) for non-sequential items
- Nested lists: 2-space indent, max 2 levels deep
- Paragraphs: single blank line between, max 3-4 sentences per paragraph

TABLE FORMATTING:
- Header: | Column A | Column B | Column C | separated by |---|---|---|
- Alignment: | :--- | left, | :---: | center, | ---: | right
- Numbers: RIGHT-aligned (---:), text LEFT-aligned (:---), headers center-aligned (:---:)
- Max 6 columns for readability -- if more needed, split into multiple tables
- Consistent decimal places and number formatting within each column
- Negative values in parentheses: ($12.4M)
- Empty cells: use -- (dash), never leave blank
- Table caption above as bold text: **Table 1: [Description]**

CODE BLOCKS:
- Always specify language tag after opening triple backticks: python, javascript, sql, etc.
- Opening comment: brief description of what the code does
- Max 30 lines per block -- break into multiple blocks with explanatory text between
- One concept per code block

PROFESSIONAL STANDARDS:
- Links: descriptive text [API Reference](url), never raw URLs in body
- Images: include alt text describing content and analytical meaning
- Consistent American English spelling and terminology throughout
- No trailing whitespace, no double blank lines, no tabs (spaces only)
- No HTML workarounds unless Markdown cannot achieve the layout
- Every claim backed by data, link, or explicit reasoning`,

    html:`Design as semantic, accessible HTML at the quality standard of Stripe's documentation or Linear's marketing pages. Production-ready, responsive, WCAG AA compliant.

DOCUMENT STRUCTURE:
- Semantic HTML5: <header>, <nav>, <main>, <article>, <section>, <aside>, <footer>
- Single <h1> for page title, <h2> for sections, <h3> for subsections -- NEVER skip heading levels
- Meta tags: charset UTF-8, viewport (width=device-width, initial-scale=1), description, author, theme-color
- Logical section ordering that reads correctly with CSS disabled
- Landmark roles on all major sections

TYPOGRAPHY & STYLING:
- CSS class names: BEM-style (.report__header, .data-table__cell--numeric, .callout--warning)
- Font stack: -apple-system, BlinkMacSystemFont, "Inter", "Segoe UI", sans-serif
- Monospace: "JetBrains Mono", "Fira Code", "SF Mono", monospace
- Body: font-size 16px, line-height 1.65, max-width 720px, color #333333
- Headings: font-weight 700, color #1B2A4A, letter-spacing -0.02em
- Color system as CSS custom properties: --color-primary: #1B2A4A; --color-accent: #4472C4; --color-text: #333; --color-muted: #666; --color-border: #E2E8F0; --color-bg-subtle: #F8F9FA
- Responsive: fluid widths, CSS Grid for layouts, stack to single column below 768px

TABLE FORMATTING:
- Structure: <table>, <caption>, <thead>, <tbody>, <tfoot> with proper <th scope="col/row">
- Wrapper: <div class="table-wrapper" style="overflow-x: auto"> for mobile scroll
- Header: font-weight 700, background var(--color-primary), color white, padding 10px 14px
- Data cells: padding 8px 14px, border-bottom 1px solid var(--color-border)
- Striped rows: tbody tr:nth-child(even) { background: var(--color-bg-subtle) }
- Numeric cells: text-align right, font-variant-numeric: tabular-nums
- <caption>: font-weight 700, text-align left, margin-bottom 8px, font-size 0.9rem
- Footnotes below table: font-size 0.8rem, color var(--color-muted)
- Column widths: no fixed widths unless necessary -- let content determine, with min-width on numeric columns

ACCESSIBILITY (WCAG AA mandatory):
- All images: descriptive alt text explaining analytical meaning, not just visual description
- Interactive elements: visible focus styles (outline: 2px solid var(--color-accent), offset 2px)
- Color contrast: minimum 4.5:1 for body text, 3:1 for large text and UI components
- ARIA: aria-label on icon buttons, aria-describedby for complex tables, aria-live for dynamic content
- Logical tab order matching visual order
- Tables: scope="col" on column headers, scope="row" on row headers
- Skip-to-content link as first focusable element
- Reduced motion: @media (prefers-reduced-motion: reduce) respecting user preference

PROFESSIONAL STANDARDS:
- Zero inline styles -- all styling via CSS classes or custom properties
- Clean 2-space indented HTML, readable without tooling
- No deprecated elements (<center>, <font>, <b> for styling -- use <strong> for semantic emphasis)
- Print stylesheet: @media print with hidden nav/footer, optimized margins, page-break rules
- Performance: suggest lazy-loading for images, minimal CSS, system fonts where possible`,

    json:`Design as production-grade JSON at the quality of a Stripe API response or Bloomberg data feed. Parseable, self-documenting, and schema-consistent.

STRUCTURE:
- Root object with clear top-level keys: "metadata", "data", "summary" (or domain-appropriate equivalents)
- Metadata object: "schemaVersion", "generatedAt" (ISO 8601), "source", "description", "recordCount"
- Consistent nesting: max 4 levels deep, flatten when possible
- Arrays for collections of same-type items (identical key shapes), objects for named properties
- Pagination-ready: include "total", "offset", "limit" in metadata when applicable

NAMING CONVENTIONS:
- camelCase for all keys: totalRevenue, createdAt, riskScore, netIncomeMargin
- Descriptive names -- NEVER abbreviate: customerLifetimeValue not clv, annualRecurringRevenue not arr
- Boolean keys: is/has/can prefix (isActive, hasRisk, canExecute)
- Arrays: plural nouns (items, metrics, risks, transactions)
- Enum values: lowercase_snake_case ("high_risk", "in_progress", "pending_review")

DATA TYPES & VALUES:
- Numbers: raw numeric, never quoted -- 1234567.89 not "1,234,567.89"
- Missing data: null (never 0, "", "N/A", or "null")
- Dates: ISO 8601 with timezone: "2026-04-09T14:30:00Z"
- Currency: { "amount": 1234.56, "currency": "USD" } -- never "$1,234.56"
- Percentages: decimal form: 0.153 not 15.3 or "15.3%"
- Booleans: true/false (never "true", 1, "yes")
- IDs: string type even if numeric (prevents integer overflow in JS)

PROFESSIONAL STANDARDS:
- Valid JSON -- parseable by JSON.parse() with zero errors
- 2-space indentation, consistent throughout
- No trailing commas, no comments
- Self-documenting: include "description" or "_comment" fields in metadata (never inline)
- Schema reference: include "$schema" or "type" field so consumers know the structure
- Every object in an array MUST have identical key sets (no missing keys -- use null for absent values)
- Sorted keys within objects for deterministic output`,

    csv:`Design as clean, production-ready CSV suitable for direct import into Excel, Python/pandas, R, or any BI tool. Data engineering quality standard.

STRUCTURE:
- Row 1: header row with descriptive, unique column names in snake_case (revenue_usd, growth_rate_pct, report_date)
- One record per row, identical column count across ALL rows (no ragged rows)
- Sort by primary analytical axis (typically date descending, or primary key ascending)
- Column order: identifiers first (id, name, ticker), then dimensions (date, region, segment), then measures (revenue, margin, growth)
- If multiple datasets needed: use separate CSV outputs, never mix schemas in one file

DATA FORMATTING:
- Strings: double-quote any field containing commas, newlines, or double quotes. Escape internal quotes with "" (RFC 4180 compliant)
- Numbers: raw numeric only, no thousand separators, decimal point (not comma): 1234567.89
- Dates: ISO 8601 (YYYY-MM-DD) for unambiguous parsing: 2026-04-09
- Timestamps: ISO 8601 with timezone: 2026-04-09T14:30:00Z
- Currency: numeric only, currency noted in column header: revenue_usd, cost_eur
- Percentages: decimal form, noted in header: growth_rate_pct (0.153 means 15.3%)
- Missing values: empty field (two adjacent commas) -- NEVER use "N/A", "null", "NULL", "-", or 0 for missing data
- Boolean: 1/0 (most tool-compatible) or TRUE/FALSE -- consistent throughout, noted in header comment

PROFESSIONAL STANDARDS:
- Encoding: UTF-8 with no BOM
- Line endings: LF (Unix) for cross-platform compatibility
- No trailing commas, no trailing whitespace on any line
- No blank rows (including at end of file)
- First line of output: brief comment block describing the dataset: # Source: [source], Date range: [range], Generated: [date], Columns: [count], Rows: [count]
- Column definitions appendix: for each column state name, data type, unit, description, and example value
- Data validation: no duplicate primary keys, no orphan foreign keys, all required fields populated`,

    plaintext:`Design as professional plain text readable in any text editor, terminal, email client, or printed monospaced output. Quality standard: a well-formatted internal memo at a top-tier firm.

STRUCTURE:
- Title block: centered title in CAPS, date (Month DD, YYYY), author, thin separator (======)
- Section headers: numbered (1.0, 1.1) in CAPS or Title Case, blank line before, underline (------) after
- Clear visual hierarchy: section > subsection > paragraph > list
- Document length marker at bottom: "--- END OF DOCUMENT ---"

LAYOUT:
- Line width: STRICT 80 characters maximum for universal compatibility
- Body text: left-aligned, single-spaced within paragraphs, one blank line between paragraphs
- Lists: dash (-) for unordered, 2-space indent for sub-items
- Numbered steps: Arabic numerals with period (1. 2. 3.), 3-space indent for continuation lines
- Tables: column-aligned using spaces, header row with dashes below, right-align numbers:
  Category        Revenue     Growth    Status
  -----------     ----------  ------    --------
  North America   $12.4M      +8.3%    On track
  Europe          $8.1M       +3.1%    Monitor
  TOTAL           $20.5M      +6.2%    On track

  Ensure columns never overlap -- add 4+ spaces between columns. Right-pad text columns to uniform width. Right-align all numeric columns.

EMPHASIS:
- CAPS for section headers and critical warnings ONLY -- never for emphasis in body text
- *asterisks* for emphasis (renders in many viewers)
- Indentation (4 spaces) for quoted text, callouts, or subordinate information
- Brackets for inline labels: [NOTE], [IMPORTANT], [ACTION REQUIRED]

PROFESSIONAL STANDARDS:
- ASCII only: use -- not em-dash, use " not smart quotes, use ... not ellipsis character
- Consistent spacing: never mix tabs and spaces, use spaces only
- Cross-references: "see Section 3.2" for longer documents
- Print-friendly: reads well on paper at any monospaced font
- NEVER let table columns overlap or misalign
- NEVER exceed 80 characters per line including indentation`,

    codeFile:`Design as a complete, production-ready code file at the quality standard of an open-source library by Google, Stripe, or Vercel. Runs without modification, fully typed, thoroughly documented.

FILE STRUCTURE:
- File header: filename, one-line description, author, date, version, license identifier (SPDX format)
- Imports grouped and ordered: (1) standard library, (2) third-party, (3) local -- blank line between groups, alphabetical within groups
- Constants and configuration immediately below imports, with explanatory comments
- Type definitions / interfaces next (if applicable)
- Helper/utility functions (private, prefixed _ in Python)
- Main public functions/classes
- Entry point / main block at bottom (if __name__ == "__main__" or equivalent)

CODE QUALITY:
- Follow the canonical style guide for the language: PEP 8 (Python), Google Style (Go/Java), Airbnb (JS/TS), rustfmt (Rust)
- Type annotations on ALL function signatures -- parameters and return types
- Docstrings/JSDoc on ALL public functions: description, @param with type and meaning, @returns, @throws, @example
- Error handling: specific exception types, meaningful messages including context (what failed, with what input), proper resource cleanup (try/finally, context managers, defer)
- Input validation at all public function boundaries with clear error messages
- No magic numbers -- use named constants with comments explaining derivation or source

NAMING CONVENTIONS:
- Functions: verb_noun pattern (calculate_revenue, fetch_user_data, validate_input)
- Classes: PascalCase, noun-based (RevenueCalculator, UserRepository)
- Constants: UPPER_SNAKE_CASE (MAX_RETRY_COUNT, DEFAULT_TIMEOUT_MS)
- Variables: descriptive snake_case or camelCase per language convention, no single-letter names except i/j/k loop counters
- Boolean variables: is_/has_/can_/should_ prefix (is_valid, has_permission)

DOCUMENTATION:
- Module-level docstring: purpose, usage example, dependencies, key assumptions
- Inline comments: only for non-obvious logic, business rules, or workarounds -- code should be self-documenting
- Usage examples: in docstrings and/or a dedicated "if __name__" or examples block at bottom
- Dependencies: noted with version constraints (>=, ~=, ^)

PROFESSIONAL STANDARDS:
- Runs without modification  -  all imports resolvable, no placeholder code
- Unit test structure suggested (test file name, key test cases to cover)
- Logging over print statements for production code
- Security: no hardcoded credentials, sanitize inputs, parameterize queries
- Performance notes where relevant (time complexity, memory considerations)`
  };
  if(fileOutput&&FILE_INST[fileOutput]){
    sec("file_output_format","Target file format: "+(FILE_OUTPUTS[fileOutput]?.l||fileOutput)+".\n\n"+FILE_INST[fileOutput]);

    // Industry-specific typography & color injection for file outputs
    ...
    if(styledFileTypes.includes(fileOutput)&&indStyle){
      ...
      let styleBlock=indStyle.label+" -- apply the following industry-standard typography and color palette throughout this document. Reference: "+indStyle.ref+".\n\n";
      styleBlock+="TYPOGRAPHY:\n";
      styleBlock+="- Primary font (headings, titles): "+f.primary+"\n";
      styleBlock+="- Secondary font (body text, narratives): "+f.secondary+"\n";
      styleBlock+="- Data font (tables, numbers, charts): "+f.data+" -- optimized for grid readability at small sizes\n";
      styleBlock+="- Monospace font (code, formulas): "+f.mono+"\n";
      styleBlock+="- If a specified font is unavailable, fall back to the closest system font in the same family (serif > serif, sans > sans)\n\n";
      styleBlock+="COLOR PALETTE:\n";
      styleBlock+="- Primary brand color: "+c.primary+" (headers, section dividers, title bars)\n";
      styleBlock+="- Secondary color: "+c.secondary+" (charts secondary series, accent elements)\n";
      styleBlock+="- Accent color: "+c.accent+" (highlights, callouts, key metrics)\n";
      styleBlock+="- Body text: "+c.text+"\n";
      styleBlock+="- Muted text (captions, footnotes): "+c.muted+"\n";
      styleBlock+="- Table header background: "+c.headerBg+" with "+c.headerText+" text\n";
      styleBlock+="- Table alternating row: "+c.altRow+"\n";
      styleBlock+="- Table borders: "+c.border+"\n";
      styleBlock+="- Negative values / losses: "+c.negative+" font color\n";
      styleBlock+="- Positive indicators / gains: "+c.positive+" font color\n\n";
      styleBlock+="Apply these fonts and colors consistently across ALL pages, slides, tabs, tables, and charts. Do not mix in other colors or fonts unless explicitly instructed. This palette defines the professional identity of the document.";
      sec("industry_design_standard",styleBlock);
    }

    // Content-aware document planning directive
    const docTypes=["pdf","word","ppt","excel"];
    if(docTypes.includes(fileOutput)){
      sec("document_planning","BEFORE writing any content, complete this document planning phase:\n\n1. CONTENT OUTLINE: Based on the task above, list every section and subsection you will create. For each section that contains a table, note: (a) the table's purpose, (b) exact column headers, (c) the widest value expected in each column, (d) total number of rows.\n\n2. TABLE DESIGN: For every table identified above:\n   - Calculate total character width of all columns (header + widest value + padding)\n   - If it exceeds the page/slide width, restructure NOW: abbreviate headers, reduce columns, split into multiple tables, or switch orientation\n   - Choose the right abbreviation scheme for the content type: fiscal years (FY24A, FY25E), financial terms (Rev., EBITDA, EV/EBITDA), ratings (H/M/L), units ($M, $B, bps)\n   - Pre-define all abbreviations in a glossary you will place at the end\n\n3. PAGE BUDGET: Estimate how many pages/slides the content will fill. Allocate space so no page is less than 40% filled. If a section would leave a mostly-blank page, either extend the content or merge with the next section.\n\n4. THEN WRITE: Only after completing steps 1-3, begin writing the actual document. Follow your plan exactly -- do not add unplanned tables or change column structures mid-document.\n\n5. FINAL VERIFICATION: After writing, review every table and confirm: (a) no cell content is truncated or broken across lines, (b) no page is mostly blank, (c) every abbreviation is defined, (d) number formatting is consistent within each column.");
    }
  }
  ...
  if(includes.length>0){
    const incInst=[];
    if(includes.includes("exec_summary"))incInst.push("Begin with an Executive Summary (3-5 sentences) stating the core findings or recommendations upfront");
    if(includes.includes("examples"))incInst.push("Include at least 2-3 real-world examples that illustrate key points concretely  -  use named companies, real scenarios, or plausible case studies");
    if(includes.includes("sources"))incInst.push("Cite sources for factual claims. Name specific reports, studies, publications, or datasets. Do not fabricate citations.");
    if(includes.includes("action_items"))incInst.push("End with a clear Action Items / Next Steps section: numbered, specific, with owners and timelines where applicable");
    if(includes.includes("risks"))incInst.push("Include a Risk Assessment section: identify top 3-5 risks, their likelihood (high/med/low), impact, and mitigation strategies");
    if(includes.includes("comparison"))incInst.push("Include a structured Comparison Table evaluating options on the same criteria. Use consistent scoring or rating.");
    if(includes.includes("timeline"))incInst.push("Include a Timeline / Milestones section with specific phases, deliverables, and estimated dates");
    if(includes.includes("metrics"))incInst.push("Define specific measurable KPIs / Metrics for success. Include baseline, target, and measurement method for each.");
    if(includes.includes("glossary"))incInst.push("Include a Glossary section defining all technical terms, acronyms, and domain-specific jargon used");
    if(includes.includes("visuals"))incInst.push("Describe recommended Visuals / Charts: what type (bar, line, flowchart, etc.), what data it shows, and why it helps the reader");
    sec("required_sections","Your response MUST include these sections:\n\n"+incInst.map((x,i)=>(i+1)+". "+x).join("\n"));
  }

  // SMART ENHANCEMENTS — injected intelligently
  ...
  if(smartTechniques.length>0){
    sec("smart_guidance","The following expert guidance has been automatically applied based on your task context:\n\n"+smartTechniques.map(e=>"- "+e.text).join("\n\n"));
  }

  // TECHNIQUES (user-selected)
  if(techniques.includes("cot"))sec("reasoning","Think through this step by step. Show your reasoning explicitly  -  walk through logic, evaluate alternatives, explain each conclusion.");
  if(techniques.includes("fewshot"))sec("examples_format",isXml?"Model your response after this pattern:\n\n<example>\n  <input>[Describe example input]</input>\n  <ideal_output>[Describe ideal output]</ideal_output>\n</example>\n\n(Replace with your own examples)":"Model your response after this pattern:\n\n**Example Input:** [Describe example input]\n**Ideal Output:** [Describe ideal output]\n\n*(Replace with your own examples)*");
  if(techniques.includes("constraints"))sec("constraints","1. Never fabricate facts, statistics, or sources\n2. If uncertain, say so explicitly with confidence level\n3. Stay tightly focused  -  no tangential drift\n4. Distinguish between established facts, analysis, and speculation\n5. Flag potentially incorrect assumptions before proceeding");
  if(techniques.includes("selfcheck"))sec("self_verification","Before submitting, verify:\n- Does it fully address every aspect of the task?\n- Is structure clear and scannable?\n- Are all claims supported?\n- Is tone consistent and audience-appropriate?\n- No unnecessary repetition or filler?\nRevise any section that fails these checks.");
  if(techniques.includes("compare"))sec("comparative_approach","Present each option with clear label and description. Evaluate on same criteria. Use structured comparison for 3+ options. State recommendation with reasoning. Note circumstances favoring alternatives.");
  if(techniques.includes("iterative"))sec("refinement","After initial draft: re-read as target audience, identify gaps or weak arguments, provide improved version, note what changed and why.");
  if(techniques.includes("roleplay"))sec("persona_depth","Embody this role fully. Draw on deep domain expertise. Use vocabulary and reasoning patterns natural to this role. Maintain consistent perspective throughout.");
  if(techniques.includes("redteam"))sec("red_team_analysis","After completing your main response, switch to adversarial mode. Adopt the perspective of a hostile critic, competitor, or skeptic and attack your own work:\n- What are the weakest arguments? Where is the evidence thinnest?\n- What assumptions could be wrong? What has been conveniently ignored?\n- How would a smart opponent dismantle this?\n- What are the second-order consequences nobody mentioned?\nPresent these challenges explicitly, then strengthen your original response to address them.");
  if(techniques.includes("firstprinciples"))sec("first_principles","Strip this problem down to its fundamental truths. Do not rely on analogy, convention, or 'how it's usually done.' Instead:\n1. What do we know to be absolutely true (physics, math, verified data)?\n2. What assumptions are we making that we haven't questioned?\n3. If we were solving this for the first time with no precedent, what would we do?\n4. Rebuild the solution from these foundations up.\nThis approach often reveals non-obvious solutions that incremental thinking misses.");
  if(techniques.includes("inversion"))sec("inversion_premortem","Before solving, invert the problem. Imagine the worst possible outcome:\n- It is 12 months from now and this has failed catastrophically. What went wrong?\n- What are the top 5 failure modes, ranked by likelihood and severity?\n- Which of these failures are we currently ignoring or underestimating?\n- For each failure mode, what early warning sign would we see?\n- What must we do NOW to prevent each failure?\nUse this pre-mortem to strengthen the actual recommendation.");
  if(techniques.includes("multiagent"))sec("multi_perspective_panel","Analyze this from multiple expert perspectives, as if convening a panel of specialists. For each perspective:\n- Name the role (e.g., CFO, customer, regulator, engineer, end user)\n- State what they would prioritize\n- Identify what they would flag as a risk or opportunity\n- Note where perspectives conflict\nSynthesize the panel's input into a recommendation that accounts for all viewpoints.");
  if(techniques.includes("structured_debate"))sec("structured_debate","Present this as a structured debate between two well-reasoned positions:\n\n**Position A (FOR):** Make the strongest possible case\n**Position B (AGAINST):** Make the strongest possible countercase\n\nFor each position: state the thesis, present 3-5 supporting arguments with evidence, address the other side's strongest point.\n\n**Verdict:** After the debate, state which position is stronger and why, with nuance about when the other position might prevail.");
  if(techniques.includes("systems"))sec("systems_thinking","Map this as a system, not an isolated problem:\n- What are the key components and how do they interact?\n- Where are the feedback loops (reinforcing and balancing)?\n- What are the leverage points where small changes produce large effects?\n- What are the time delays between action and consequence?\n- What unintended side effects could emerge?\nDraw connections that a narrow analysis would miss.");
  if(techniques.includes("decompose"))sec("task_decomposition","Break this complex task into clearly defined sub-tasks:\n1. List every distinct sub-problem that must be solved\n2. Identify dependencies between sub-tasks (what must happen first?)\n3. For each sub-task, state the approach, expected output, and success criteria\n4. Solve each sub-task systematically\n5. Integrate the sub-solutions into a coherent whole\n6. Verify the integrated solution against the original requirements");
  if(techniques.includes("meta"))sec("meta_reasoning","Before diving in, think about HOW to think about this problem:\n- What type of problem is this? (optimization, trade-off, diagnosis, design, prediction)\n- What frameworks or mental models are most applicable?\n- What is the biggest risk of getting this wrong?\n- Where am I most likely to have blind spots?\n- What level of confidence can I realistically achieve?\nLet this meta-analysis guide your approach, and revisit it if you get stuck.");

  // EXTRA
  if(extra)sec("additional_instructions",extra);

  // SPECIAL REQUESTS
  if(special)sec("special_requests_and_constraints",special);

  // DOCUMENT QUALITY GATE — final check for file outputs (skip for fast tier)
  const docFileTypes=["pdf","word","ppt","excel"];
  if(tier!=="fast"&&fileOutput&&docFileTypes.includes(fileOutput)){
    sec("quality_gate","BEFORE delivering your final response, perform this quality gate check on every table, matrix, and structured element:\n\n1. CELL INTEGRITY: Read every cell in every table. Does any cell contain a word that is split across two lines? If yes, abbreviate the header or widen the column.\n2. COLUMN FIT: Is any column so narrow that its content wraps? If yes, redistribute column widths.\n3. PAGE UTILIZATION: Is any page less than 40% filled with content? If yes, merge sections or extend content to fill it.\n4. NUMBER CONSISTENCY: Within each column, are all numbers formatted identically (same decimal places, same unit notation, same alignment)? Fix any inconsistency.\n5. ABBREVIATION COVERAGE: Is every abbreviation defined in the glossary or a footnote? Add any missing definitions.\n\nIf ANY check fails, fix it before delivering. The document must look like it was prepared by a senior analyst at a top-tier investment bank for a board presentation.");
  }

  // ANTI-PATTERNS (condensed for fast tier)
  if(tier==="fast"){
    sec("avoid","- No filler phrases, hedging, or preamble. Start with substance, end with actionable specifics.");
  }else{
    sec("avoid","- Do not open with filler phrases like \"Sure!\" or \"Great question!\"  -  begin directly with substance\n- Do not use vague hedging like \"it depends\" without explaining what it depends on specifically\n- Do not pad with generic background the audience already knows\n- Do not end with empty cliches  -  close with actionable specifics\n- Do not repeat the same point in different words to inflate length");
  }

  // TOKEN EFFICIENCY for non-flagship tiers
  if(tier==="balanced"){
    sec("efficiency","This prompt targets a balanced-tier model. Prioritize clarity and completeness but avoid over-elaboration. Use concise sentences. Combine related points where possible. Aim for maximum insight per paragraph.");
  }else if(tier==="fast"){
    sec("efficiency","This prompt targets a fast/lightweight model. Be extremely concise. Use short sentences, bullet points over paragraphs, and skip obvious context. Prioritize the single most important answer. Keep total response short.");
  }
...
async function loadHistoryFromDB(){
  ...
  return (data||[]).map(r=>({id:r.id,topic:r.topic,task:r.task,industry:r.industry,output:r.output,model:r.model,fileOutput:r.file_output,timestamp:new Date(r.created_at).getTime()}));
}
async function saveHistoryToDB(entry){
  const {data:{user}}=await supabase.auth.getUser();
  if(!user)return;
  const {error}=await supabase.from("prompt_history").insert({user_id:user.id,topic:entry.topic,task:entry.task,industry:entry.industry,output:entry.output,model:entry.model,file_output:entry.fileOutput||"pdf",prompt_text:entry.promptText||null});
...
function App(){
  ...
  const [fileOutput,setFileOutput]=useState("pdf");
  ...
  const go=useCallback(()=>{
    ...
    if(user){
      const entry={topic,task,industry,output,model,fileOutput,promptText:prompt};
  ...
  },[topic,task,industry,output,model,fileOutput,prompt,user,isExp,usage]);
  ...
  const loadExample=useCallback((ex)=>{setTopic(ex.topic);setTask(ex.task);setIndustry(ex.industry);setOutput(ex.output);setModel(ex.model);setSelectedFirm("");setSelectedRole("");setShow(false);},[]);

  const loadFromHistory=useCallback((h)=>{setTopic(h.topic);setTask(h.task);setIndustry(h.industry);setOutput(h.output);setModel(h.model);setShowHistory(false);setShow(false);},[]);
    ...
    <div style={{minHeight:"100vh",background:"var(--bg)",color:"var(--t1)",fontFamily:"var(--f)",paddingTop:(!user||!usage.is_paid)?84:56}}>
      ...
      {showHistory&&history.length>0&&(
        <div style={{background:"var(--s1)",borderRadius:14,border:"1px solid var(--bd)",padding:20,marginBottom:24,animation:"fadeIn .2s ease",boxShadow:"0 1px 4px rgba(0,0,0,.04)"}}>
          ...
          {history.map((h,i)=>(
            <button key={i} onClick={()=>loadFromHistory(h)} style={{display:"block",width:"100%",textAlign:"left",padding:"10px 12px",marginBottom:4,borderRadius:8,border:"1px solid var(--bd)",background:"var(--s2)",color:"var(--t1)",cursor:"pointer",fontFamily:"var(--f)",fontSize:12,lineHeight:1.4,transition:"border-color .15s"}}>
              <div style={{fontWeight:600,marginBottom:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{h.topic}</div>
              <div style={{color:"var(--t3)",fontSize:11}}>{tl(UI_TASKS,h.task)} \u00b7 {tl(UI_INDUSTRIES,h.industry)} \u00b7 {new Date(h.timestamp).toLocaleDateString()}</div>
            </button>
          ))}
        </div>
      )}
      ...
      {show&&topic.trim()&&(
        <div ref={ref} style={{background:"var(--s1)",borderRadius:16,border:"1px solid var(--bd)",overflow:"hidden",animation:"up .35s ease",boxShadow:"0 2px 8px rgba(0,0,0,.06)"}}>
          ...
          <div style={{padding:"10px 20px",borderTop:"1px solid var(--bd)",display:"flex",gap:14,flexWrap:"wrap",fontSize:11,color:"var(--t3)"}}>
            ...
            <span style={{display:"flex",alignItems:"center",gap:3}}>{I.download(10,"var(--t3)")} {tl(UI_FILE_OUTPUTS,fileOutput)}</span>
