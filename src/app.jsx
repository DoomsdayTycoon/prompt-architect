...
const UI={
  ...
  refTitle:{en:"Give $5, get $5",no:"Gi 5 USD, få 5 USD"},
  refSub:{en:"Share your referral link. When someone signs up and pays, you both get $5 off your next invoice — credit is applied automatically.",no:"Del din henvisningslenke. Når noen melder seg på og betaler, får dere begge 5 USD i rabatt på neste faktura — kreditten trekkes automatisk."},
  refShareX:{en:"Share on X",no:"Del på X"},
  refShareLi:{en:"Share on LinkedIn",no:"Del på LinkedIn"},
  refTweet:{en:"I use Prompt Architect to turn vague ideas into expert AI prompts. Sign up through my link and we both get $5 off:",no:"Jeg bruker Prompt Architect for å gjøre vage ideer til ekspertprompter. Meld deg på via lenken min og vi får begge 5 USD i rabatt:"},
  refClose:{en:"Close",no:"Lukk"},
  refCopy:{en:"Copy",no:"Kopier"},
  refCopied:{en:"Copied!",no:"Kopiert!"},
  refInviteBanner:{en:"You've been invited — $5 credit will be applied when you sign up and subscribe.",no:"Du har blitt invitert — 5 USD kreditt legges til når du melder deg på og abonnerer."},
...
const IND_CONTEXT={
  ...
  ecommerce:"You operate in e-commerce and digital retail. Use industry metrics (AOV, conversion rate, cart abandonment, CLV). Reference platform-specific best practices (leading hosted platforms, open-source carts, headless commerce). Consider UX/UI, mobile-first design, payment flows, and logistics. Address SEO, merchandising, and personalization strategies.",
...
function buildPrompt(p){
  ...
  // Methodology deliverables — task-type-specific bullet list that tells
  // the model exactly how to structure the work. This gives generated
  // prompts the concrete, bullet-by-bullet structure of reference-quality
  // finance/consulting prompts rather than a generic "do a good job".
  if(METHODOLOGY[taskType]&&METHODOLOGY[taskType].length>0){
    sec("methodology",METHODOLOGY[taskType].map(x=>"- "+x).join("\n"));
  }
  if(QUALITY[taskType]){
    sec("quality_bar",QUALITY[taskType]);
  }
...
function App(){
  ...
  const [referralCents,setReferralCents]=useState(0);
  ...
  useEffect(()=>{
    if(!refCode||authLoading||user)return;
    setAuthMode("signup");
    setShowAuth(true);
    setAuthErr("");
  },[refCode,authLoading,user]);

  // Pointer glow — soft radial dot that follows the cursor with easing, grows on
  // interactive hover. Mirrors the reference design exactly (60px → 90px, .18 easing).
  // Touch devices skip the listeners entirely (CSS also hides the element).
  useEffect(()=>{
    if(typeof window==="undefined")return;
    const dot=document.getElementById("pa-cursor-dot");
    if(!dot)return;
    if(window.matchMedia&&window.matchMedia("(hover:none),(pointer:coarse)").matches)return;
    const body=document.body;
    let mx=window.innerWidth/2,my=window.innerHeight/2,cx=mx,cy=my,raf=0;
    const onMove=e=>{mx=e.clientX;my=e.clientY;if(!body.classList.contains("pa-cursor-ready"))body.classList.add("pa-cursor-ready");};
    const onLeave=()=>body.classList.add("pa-cursor-hidden");
    const onEnter=()=>body.classList.remove("pa-cursor-hidden");
    const loop=()=>{cx+=(mx-cx)*.18;cy+=(my-cy)*.18;dot.style.left=cx+"px";dot.style.top=cy+"px";raf=requestAnimationFrame(loop);};
    const addHover=()=>body.classList.add("pa-cursor-hover");
    const removeHover=()=>body.classList.remove("pa-cursor-hover");
    // Event delegation so nodes added/removed later (modals, dropdowns) still trigger hover scaling.
    const onOver=e=>{if(e.target.closest&&e.target.closest("a,button,[role=button],input,textarea,select,.wow-fcard,.wow-pcard,.wow-mcard,.wow-tpill"))addHover();};
    const onOut=e=>{if(e.target.closest&&e.target.closest("a,button,[role=button],input,textarea,select,.wow-fcard,.wow-pcard,.wow-mcard,.wow-tpill"))removeHover();};
    ...
    return ()=>{cancelAnimationFrame(raf);document.removeEventListener("mousemove",onMove);document.removeEventListener("mouseleave",onLeave);document.removeEventListener("mouseenter",onEnter);document.removeEventListener("mouseover",onOver);document.removeEventListener("mouseout",onOut);body.classList.remove("pa-cursor-ready","pa-cursor-hover","pa-cursor-hidden");};
  },[]);
  ...
  useEffect(()=>{
    if(!showReferral||!user?.id)return;
    supabase.from("profiles").select("referral_credits_cents").eq("id",user.id).single().then(({data})=>{
      if(data&&typeof data.referral_credits_cents==="number")setReferralCents(data.referral_credits_cents);
    });
  ...
  return <>
    <style>{`
      ...
      *{box-sizing:border-box}::-webkit-scrollbar{width:6px}::-webkit-scrollbar-thumb{background:#cbd5e1;border-radius:3px}::-webkit-scrollbar-track{background:transparent}
      ...
      #pa-cursor-dot{position:fixed;top:0;left:0;width:60px;height:60px;border-radius:50%;background:radial-gradient(circle,${ac}1A 0%,transparent 70%);pointer-events:none;z-index:9999;transform:translate(-50%,-50%);transition:width .3s,height .3s,opacity .3s;will-change:left,top;opacity:0}
      body.pa-cursor-ready #pa-cursor-dot{opacity:1}
      body.pa-cursor-hover #pa-cursor-dot{width:90px;height:90px}
      body.pa-cursor-hidden #pa-cursor-dot{opacity:0}
      @media(hover:none),(pointer:coarse){#pa-cursor-dot{display:none}}
    `}</style>
    <div id="pa-cursor-dot" aria-hidden="true"/>
    {/* TOP OFFER BANNER */}
    ...
    {showAuth&&(
      <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,.45)",backdropFilter:"blur(4px)",WebkitBackdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:2000,padding:20}} onClick={()=>setShowAuth(false)}>
        <div style={{background:"var(--s1)",borderRadius:18,padding:36,maxWidth:400,width:"100%",boxShadow:"0 24px 64px rgba(0,0,0,.15)",animation:"slideDown .3s ease"}} onClick={(e)=>e.stopPropagation()}>
          ...
          {authMode==="signup"&&refCode&&<div style={{background:"#10B98112",border:"1px solid #10B98130",borderRadius:10,padding:"10px 14px",margin:"0 0 18px",display:"flex",alignItems:"center",gap:10,fontSize:12.5,color:"#065F46",fontWeight:500,lineHeight:1.5}}>{I.gift(18,"#10B981")}<span>{t("refInviteBanner")}</span></div>}
    ...
    {showReferral&&(
      <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:2000,padding:20}} onClick={()=>setShowReferral(false)}>
        <div style={{background:"var(--s1)",borderRadius:16,padding:32,maxWidth:420,width:"100%",boxShadow:"0 20px 60px rgba(0,0,0,.15)",animation:"slideDown .25s ease"}} onClick={(e)=>e.stopPropagation()}>
          ...
          <p style={{fontSize:13,color:"var(--t2)",margin:"0 0 16px",textAlign:"center",lineHeight:1.6}}>{t("refSub")}</p>
          {referralCents>0&&<div style={{background:"#10B98110",border:"1px solid #10B98130",borderRadius:10,padding:"10px 14px",marginBottom:16,textAlign:"center",fontSize:13,color:"#065F46",fontWeight:600}}>{uiLang==="no"?`Du har tjent $${(referralCents/100).toFixed(2)} i kreditt`:`You've earned $${(referralCents/100).toFixed(2)} in credit`}</div>}
